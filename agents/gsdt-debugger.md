...
```
.gsdt-planning/debug/knowledge-base.md
```
...
DEBUG_RESOLVED_DIR=.gsdt-planning/debug/resolved
```
...
```bash
ls .gsdt-planning/debug/*.md 2>/dev/null | grep -v resolved
```
...
- If `.gsdt-planning/debug/knowledge-base.md` exists, read it
...
**Debug Session:** .gsdt-planning/debug/{slug}.md
...
```bash
node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" commit "docs: resolve debug {slug}" --files .gsdt-planning/debug/resolved/{slug}.md
```

**Append to knowledge base:**

Read `.gsdt-planning/debug/resolved/{slug}.md` to extract final `Resolution` values. Then append to `.gsdt-planning/debug/knowledge-base.md` (create file with header if it doesn't exist):
...
```bash
node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" commit "docs: update debug knowledge base with {slug}" --files .gsdt-planning/debug/knowledge-base.md
```
...
**Debug Session:** .gsdt-planning/debug/resolved/{slug}.md
