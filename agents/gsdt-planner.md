...
<output>
After completion, create `.gsdt-planning/phases/XX-name/{phase}-{plan}-SUMMARY.md`
</output>
```
...
```bash
cat .gsdt-planning/phases/$PHASE-*/$PHASE-*-PLAN.md
```
...
```bash
node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" commit "fix($PHASE): revise plans based on checker feedback" --files .gsdt-planning/phases/$PHASE-*/$PHASE-*-PLAN.md
```
...
- .gsdt-planning/phases/16-xxx/16-01-PLAN.md
- .gsdt-planning/phases/16-xxx/16-02-PLAN.md
...
```bash
cat .gsdt-planning/STATE.md 2>/dev/null
```

If STATE.md missing but .gsdt-planning/ exists, offer to reconstruct or continue without.
</step>
...
```bash
ls .gsdt-planning/codebase/*.md 2>/dev/null
```
...
```bash
cat .gsdt-planning/phases/{selected-phase}/*-SUMMARY.md
```
...
```bash
cat .gsdt-planning/RETROSPECTIVE.md 2>/dev/null | tail -100
```
...
Write to `.gsdt-planning/phases/XX-name/{phase}-{NN}-PLAN.md`
...
1. Read `.gsdt-planning/ROADMAP.md`
...
```bash
node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" commit "docs($PHASE): create phase plan" --files .gsdt-planning/phases/$PHASE-*/$PHASE-*-PLAN.md .gsdt-planning/ROADMAP.md
```
</step>
