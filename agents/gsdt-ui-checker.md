---
name: gsdt-ui-checker
description: Validates UI-SPEC.md design contracts against 6 quality dimensions. Produces BLOCK/FLAG/PASS verdicts. Spawned by /gsdt:ui-phase orchestrator.
...
<upstream_input>
**UI-SPEC.md** — Design contract from gsdt-ui-researcher (primary input)
...
> Skip this dimension entirely if `workflow.ui_safety_gate` is explicitly set to `false` in `.claude/.gsdt-planning/config.json`. If the key is absent, treat as enabled.
...
fix_hint: "Re-run /gsdt:ui-phase to trigger the registry vetting gate, or manually run 'npx shadcn view {block} --registry {url}' and record results"
```
```yaml
dimension: 6
severity: PASS
...
### Action Required
Fix blocking issues in UI-SPEC.md and re-run `/gsdt:ui-phase`.
```
