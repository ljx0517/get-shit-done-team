...
In `.gsdt-planning/config.json`, list sub-repo directories under `planning.sub_repos`:
...
4. **Planning stays local:** The `.gsdt-planning/` directory is not committed; it acts as cross-repo coordination.
...
```bash
node ~/.claude/gsdt/bin/gsdt-tools.cjs commit-to-subrepo "feat(02-01): add user API" \
