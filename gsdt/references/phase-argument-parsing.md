...
```bash
PHASE_INFO=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" roadmap get-phase "${PHASE}" --pick found)
...
```bash
PHASE_DIR=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" find-phase "${PHASE}" --raw)
```
