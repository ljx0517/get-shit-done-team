<purpose>
Generate a UI design contract (UI-SPEC.md) for frontend phases. Orchestrates gsdt-ui-researcher and gsdt-ui-checker with a revision loop. Inserts between discuss-phase and plan-phase in the lifecycle.
...
AGENT_SKILLS_UI=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" agent-skills gsdt-ui-researcher 2>/dev/null)
AGENT_SKILLS_UI_CHECKER=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" agent-skills gsdt-ui-checker 2>/dev/null)
```
...
```bash
UI_RESEARCHER_MODEL=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" resolve-model gsdt-ui-researcher --raw)
UI_CHECKER_MODEL=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" resolve-model gsdt-ui-checker --raw)
```

Check config:

```bash
UI_ENABLED=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" config-get workflow.ui_phase 2>/dev/null || echo "true")
```

**If `UI_ENABLED` is `false`:**
```
UI phase is disabled in config. Enable via /gsdt:settings.
```
Exit workflow.
...
Recommended: run /gsdt:discuss-phase {N} first to capture design preferences.
...
```markdown
Read ~/.claude/agents/gsdt-ui-researcher.md for instructions.
...
Template: ~/.claude/gsdt/templates/UI-SPEC.md
</output>
...
Task(
  ...
  subagent_type="gsdt-ui-researcher",
...
```markdown
Read ~/.claude/agents/gsdt-ui-checker.md for instructions.
...
- Re-spawn gsdt-ui-researcher with revision context:
...
2. Edit manually — open UI-SPEC.md in editor, re-run /gsdt:ui-phase
...
```bash
node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" commit "docs(${padded_phase}): UI design contract" --files "${PHASE_DIR}/${PADDED_PHASE}-UI-SPEC.md"
```
...
- [ ] gsdt-ui-researcher spawned with correct context and file paths
- [ ] UI-SPEC.md created in correct location
- [ ] gsdt-ui-checker spawned with UI-SPEC.md
