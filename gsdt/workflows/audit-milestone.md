...
AGENT_SKILLS_CHECKER=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" agent-skills gsdt-integration-checker 2>/dev/null)
```
...
```bash
integration_checker_model=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" resolve-model gsdt-integration-checker --raw)
```
...
node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" phases list
```
...
PHASE_INFO=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" find-phase 01 --raw)
...
${AGENT_SKILLS_CHECKER}",
  subagent_type="gsdt-integration-checker",
...
```bash
NYQUIST_CONFIG=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" config-get workflow.nyquist_validation --raw 2>/dev/null)
```
...
Discovery only — never auto-calls `/gsdt:validate-phase`.

## 6. Aggregate into v{version}-MILESTONE-AUDIT.md

Create `.gsdt-planning/v{version}-v{version}-MILESTONE-AUDIT.md` with:
...
**Report:** .gsdt-planning/v{version}-MILESTONE-AUDIT.md
...
|-------|---------------|-----------|--------|
| {phase} | exists/missing | true/false/partial | `/gsdt:validate-phase {N}` |

Phases needing validation: run `/gsdt:validate-phase {N}` for each flagged phase.
...
**Also available:**
- cat .gsdt-planning/v{version}-MILESTONE-AUDIT.md — see full report
