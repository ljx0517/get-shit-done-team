<purpose>
Create an isolated workspace directory with git repo copies (worktrees or clones) and an independent `.gsdt-planning/` directory. Supports multi-repo orchestration and single-repo feature branch isolation.
</purpose>
...
```bash
INIT=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" init new-workspace)
...
Run this command from a directory containing git repos, or specify repos explicitly:
  /gsdt:new-workspace --name my-workspace --repos /path/to/repo1,/path/to/repo2
```
Exit.
...
Usage:
  /gsdt:new-workspace --name my-workspace --repos repo1,repo2 --auto
```
Exit.
...
## 8. Initialize .gsdt-planning/

```bash
mkdir -p "$TARGET_PATH/.gsdt-planning"
```
...
- [ ] `.gsdt-planning/` directory initialized at workspace root
