...
- Check for MILESTONE-CONTEXT.md (from /gsdt:discuss-milestone)
...
**After each phase transition** (via `/gsdt:transition`):
...
**After each milestone** (via `/gsdt:complete-milestone`):
...
```bash
node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" commit "docs: start milestone v[X.Y] [Name]" --files .gsdt-planning/PROJECT.md .gsdt-planning/STATE.md
```

## 7. Load Context and Resolve Models

```bash
INIT=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" init new-milestone)
...
find .gsdt-planning/phases -mindepth 1 -maxdepth 1 -type d -exec mv {} "${phase_archive_path}/" \;
```

Then verify `.gsdt-planning/phases/` no longer contains old milestone directories before continuing.
...
- Tell the user to complete/archive the previous milestone first, then rerun `/gsdt:new-milestone --reset-phase-numbers ${GSD_WS}`.
...
**IMPORTANT:** Do NOT persist this choice to config.json. The `workflow.research` setting is a persistent user preference that controls plan-phase behavior across the project. Changing it here would silently alter future `/gsdt:plan-phase` behavior. To change the default, use `/gsdt:settings`.
...
<output>
Write to: .gsdt-planning/research/{FILE}
...
**Commit requirements:**
```bash
node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" commit "docs: define milestone v[X.Y] requirements" --files .gsdt-planning/REQUIREMENTS.md
```
...
- .gsdt-planning/research/SUMMARY.md (if exists)
- .gsdt-planning/config.json
- .gsdt-planning/MILESTONES.md
</files_to_read>
...
", subagent_type="gsdt-roadmapper", model="{roadmapper_model}", description="Create roadmap")
```
...
```bash
node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" commit "docs: create milestone v[X.Y] roadmap ([N] phases)" --files .gsdt-planning/ROADMAP.md .gsdt-planning/STATE.md .gsdt-planning/REQUIREMENTS.md
```
...
`/gsdt:discuss-phase [N] ${GSD_WS}` — gather context and clarify approach

<sub>`/clear` first → fresh context window</sub>

Also: `/gsdt:plan-phase [N] ${GSD_WS}` — skip discussion, plan directly
```
...
- [ ] gsdt-roadmapper spawned with phase numbering context
- [ ] Roadmap files written immediately (not draft)
- [ ] User feedback incorporated (if any)
- [ ] Phase numbering mode respected (continued or reset)
- [ ] All commits made (if planning docs committed)
- [ ] User knows next step: `/gsdt:discuss-phase [N] ${GSD_WS}`
