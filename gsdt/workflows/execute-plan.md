...
@~/.claude/gsdt/references/git-integration.md
</required_reading>
...
```bash
INIT=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" init execute-phase "${PHASE}")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_EXECUTOR=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" agent-skills gsdt-executor 2>/dev/null)
```
...
If `.gsdt-planning/` missing: error.
</step>

<step name="identify_plan">
```bash
# Use plans/summaries from INIT JSON, or list files
(ls .gsdt-planning/phases/XX-name/*-PLAN.md 2>/dev/null || true) | sort
(ls .gsdt-planning/phases/XX-name/*-SUMMARY.md 2>/dev/null || true) | sort
```
...
# config settings can be fetched via gsdt-tools config-get if needed
```
...
```bash
grep -n "type=\"checkpoint" .gsdt-planning/phases/XX-name/{phase}-{plan}-PLAN.md
```
...
**Pattern A:** init_agent_tracking → spawn Task(subagent_type="gsdt-executor", model=executor_model, isolation="worktree") with prompt: execute plan at [path], autonomous, all tasks + SUMMARY + commit, follow deviation/auth rules, report: plan name, tasks, SUMMARY path, commit hash, and append `${AGENT_SKILLS_EXECUTOR}` so executor-specific project skills are loaded → track agent_id → wait → update tracking → report.
...
```bash
if [ ! -f .gsdt-planning/agent-history.json ]; then
  echo '{"version":"1.0","max_entries":50,"entries":[]}' > .gsdt-planning/agent-history.json
fi
rm -f .gsdt-planning/current-agent-id.txt
if [ -f .gsdt-planning/current-agent-id.txt ]; then
  INTERRUPTED_ID=$(cat .gsdt-planning/current-agent-id.txt)
...
2. Per segment:
   - Subagent route: spawn gsdt-executor for assigned tasks only. Prompt: task range, plan path, read full plan for context, execute assigned tasks, track deviations, NO SUMMARY/commit, and append `${AGENT_SKILLS_EXECUTOR}`. Track via agent protocol.
...
```bash
cat .gsdt-planning/phases/XX-name/{phase}-{plan}-PLAN.md
```
...
```bash
node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" phases list --type summaries --raw
...
See `~/.claude/gsdt/references/tdd.md` for structure.
</tdd_plan_execution>
...
```bash
node ~/.claude/gsdt/bin/gsdt-tools.cjs commit-to-subrepo "{type}({phase}-{plan}): {description}" --files file1 file2 ...
```
...
See ~/.claude/gsdt/references/checkpoints.md for details.
</step>
...
```bash
NODE_REPAIR=$(node "./.claude/gsdt/bin/gsdt-tools.cjs" config-get workflow.node_repair 2>/dev/null || echo "true")
```

If `NODE_REPAIR` is `true`: invoke `@./.claude/gsdt/workflows/node-repair.md` with:
...
```bash
grep -A 50 "^user_setup:" .gsdt-planning/phases/XX-name/{phase}-{plan}-PLAN.md | head -50
```

If user_setup exists: create `{phase}-USER-SETUP.md` using template `~/.claude/gsdt/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>

<step name="create_summary">
Create `{phase}-{plan}-SUMMARY.md` at `.gsdt-planning/phases/XX-name/`. Use `~/.claude/gsdt/templates/summary.md`.
...
Update STATE.md using gsdt-tools:

```bash
# Advance plan counter (handles last-plan edge case)
node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" state advance-plan

# Recalculate progress bar from disk state
node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" state update-progress

# Record execution metrics
node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" state record-metric \
...
node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" state add-decision \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found
node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>

<step name="update_session_continuity">
Update session info using gsdt-tools:

```bash
node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" state record-session \
...
```bash
node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" roadmap update-plan-progress "${PHASE}"
```
...
```bash
node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" requirements mark-complete ${REQ_IDS}
```
...
```bash
node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" commit "docs({phase}-{plan}): complete [plan-name] plan" --files .gsdt-planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .gsdt-planning/STATE.md .gsdt-planning/ROADMAP.md .gsdt-planning/REQUIREMENTS.md
```
</step>

<step name="update_codebase_map">
If .gsdt-planning/codebase/ doesn't exist: skip.
...
|-----------|-------|--------|
| summaries < plans | **A: More plans** | Find next PLAN without SUMMARY. Yolo: auto-continue. Interactive: show next plan, suggest `/gsdt:execute-phase {phase}` + `/gsdt:verify-work`. STOP here. |
| summaries = plans, current < highest phase | **B: Phase done** | Show completion, suggest `/gsdt:plan-phase {Z+1}` + `/gsdt:verify-work {Z}` + `/gsdt:discuss-phase {Z+1}` |
| summaries = plans, current = highest phase | **C: Milestone done** | Show banner, suggest `/gsdt:complete-milestone` + `/gsdt:verify-work` + `/gsdt:add-phase` |
