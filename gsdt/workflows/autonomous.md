...
**If `roadmap_exists` is false:** Error — "No ROADMAP.md found. Run `/gsdt:new-milestone` first."
**If `state_exists` is false:** Error — "No STATE.md found. Run `/gsdt:new-milestone` first."
...
```bash
DETAIL=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" roadmap get-phase ${PHASE_NUM})
```
...
```bash
SKIP_DISCUSS=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" config-get workflow.skip_discuss 2>/dev/null || echo "false")
```
...
```bash
node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" commit "docs(${PADDED_PHASE}): auto-generated context (discuss skipped)" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
PHASE_SECTION=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" roadmap get-phase ${PHASE_NUM} 2>/dev/null)
...
```bash
UI_REVIEW_CFG=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" config-get workflow.ui_review 2>/dev/null || echo "true")
```
...
> **Note:** Smart discuss is an autonomous-optimized variant of the `gsdt:discuss-phase` skill. It produces identical CONTEXT.md output but uses batch table proposals instead of sequential questioning. The original `discuss-phase` skill remains unchanged (per CTRL-03). Future milestones may extract this to a separate skill file.
...
```bash
node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" commit "docs(${PADDED_PHASE}): smart discuss context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
On **"Stop"**: Go to handle_blocker with "User stopped — audit gaps remain. Run /gsdt:audit-milestone to review, then /gsdt:complete-milestone when ready."
...
On **"Stop"**: Go to handle_blocker with "User stopped — tech debt to address. Run /gsdt:audit-milestone to review details."
...
── GSDT ► AUTONOMOUS ▸ STOPPED ──
 ...
 Resume with: /gsdt:autonomous --from {next_phase}
```
