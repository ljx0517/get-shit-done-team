...
Seeds auto-surface during /gsdt:new-milestone when trigger conditions match the
...
This seed should be presented during `/gsdt:new-milestone` when the milestone
...
```bash
node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" commit "docs: plant seed — {$IDEA}" --files .planning/seeds/SEED-{PADDED}-{slug}.md
```
</step>
