...
**Critical**: `list` is only a subcommand when it's the ENTIRE argument. `/gsdt:note list of groceries` saves a note with text "list of groceries". Same for `promote` — only a subcommand when followed by exactly one number.
</step>
...
{count} active note(s). Use `/gsdt:note promote <N>` to convert to a todo.
```
...
4. **Requires `.planning/` directory** — if it doesn't exist, warn: "Todos require a GSD project. Run `/gsdt:new-project` to initialize one."
...
status: pending
priority: P2
source: "promoted from /gsdt:note"
created: {YYYY-MM-DD}
theme: general
---
...
3. **Promote without project**: Warns that todos require `.planning/`, suggests `/gsdt:new-project`
