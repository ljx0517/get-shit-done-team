...
1. Check `.gsdt-planning/STATE.md` for current milestone version
2. Check `.gsdt-planning/milestones/` for the latest archived version
3. If neither found, check if `.gsdt-planning/ROADMAP.md` exists (project may be mid-milestone)
...
**Archived milestone** (`.gsdt-planning/milestones/v{VERSION}-ROADMAP.md` exists):
```
ROADMAP_PATH=".gsdt-planning/milestones/v${VERSION}-ROADMAP.md"
REQUIREMENTS_PATH=".gsdt-planning/milestones/v${VERSION}-REQUIREMENTS.md"
AUDIT_PATH=".gsdt-planning/milestones/v${VERSION}-MILESTONE-AUDIT.md"
```

**Current/in-progress milestone** (no archive yet):
```
ROADMAP_PATH=".gsdt-planning/ROADMAP.md"
REQUIREMENTS_PATH=".gsdt-planning/REQUIREMENTS.md"
AUDIT_PATH=".gsdt-planning/v${VERSION}-MILESTONE-AUDIT.md"
```

Note: The audit file moves to `.gsdt-planning/milestones/` on archive (per `complete-milestone` workflow). Check both locations as a fallback.

**Always available:**
```
PROJECT_PATH=".gsdt-planning/PROJECT.md"
RETRO_PATH=".gsdt-planning/RETROSPECTIVE.md"
STATE_PATH=".gsdt-planning/STATE.md"
```
...
Find the earliest `.gsdt-planning/phases/` commit:
```bash
git log --oneline --diff-filter=A -- ".gsdt-planning/phases/" | tail -1
```
...
Write to `.gsdt-planning/reports/MILESTONE_SUMMARY-v${VERSION}.md`:
...
**Overwrite guard:** If `.gsdt-planning/reports/MILESTONE_SUMMARY-v${VERSION}.md` already exists, ask the user:
...
> "Summary written to `.gsdt-planning/reports/MILESTONE_SUMMARY-v{VERSION}.md`.
>
...
If the user is done:
- Suggest next steps: `/gsdt:new-milestone`, `/gsdt:progress`, or sharing the summary with the team
...
gsdt-tools.cjs state record-session \
  ...
  --resume-file ".gsdt-planning/reports/MILESTONE_SUMMARY-v${VERSION}.md"
```
