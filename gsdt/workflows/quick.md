...
```
── GSDT ► QUICK TASK (DISCUSS + RESEARCH + FULL) ──
...
```
── GSDT ► QUICK TASK (DISCUSS + RESEARCH) ──
...
```bash
INIT=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" init quick "$DESCRIPTION")
...
AGENT_SKILLS_VERIFIER=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" agent-skills gsdt-verifier 2>/dev/null)
```

Parse JSON for: `planner_model`, `executor_model`, `checker_model`, `verifier_model`, `commit_docs`, `branch_name`, `quick_id`, `slug`, `date`, `timestamp`, `quick_dir`, `task_dir`, `roadmap_exists`, `planning_exists`.

**If `roadmap_exists` is false:** Error — Quick mode requires an active project with ROADMAP.md. Run `/gsdt:new-project` first.
...
Spawn gsdt-executor with plan reference:
...
```bash
node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" commit "docs(quick-${quick_id}): ${DESCRIPTION}" --files ${file_list}
```
...
GSDT > QUICK TASK COMPLETE (FULL MODE)
...
Ready for next task: /gsdt:quick ${GSD_WS}
```
