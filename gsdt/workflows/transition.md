...
There is no `/gsdt:transition` command. This workflow is invoked automatically by
`execute-phase` during auto-advance, or inline by the orchestrator after phase
verification. Users should never be told to run `/gsdt:transition`.

**Valid user commands for phase progression:**
- `/gsdt:discuss-phase {N}` — discuss a phase before planning
- `/gsdt:plan-phase {N}` — plan a phase
- `/gsdt:execute-phase {N}` — execute a phase
- `/gsdt:progress` — see roadmap progress
...
These will carry forward as debt. Review: `/gsdt:audit-uat`
```
...
**Delegate ROADMAP.md and STATE.md updates to gsdt-tools:**

```bash
TRANSITION=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" phase complete "${current_phase}")
```
...
**Note:** Basic position updates (Current Phase, Status, Current Plan, Last Activity) were already handled by `gsdt-tools phase complete` in the update_roadmap_and_state step.

Verify the updates are correct by reading STATE.md. If the progress bar needs updating, use:

```bash
PROGRESS=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" progress bar --raw)
```
...
**Use the transition result from `gsdt-tools phase complete`:**
...
if [ -n "$GSD_WORKSTREAM" ]; then
  WS_LIST=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" workstream list --raw)
fi
```
...
`/gsdt:discuss-phase [X+1] ${GSD_WS}` — gather context and clarify approach
...
**Also available:**
- `/gsdt:plan-phase [X+1] ${GSD_WS}` — skip discussion, plan directly
- `/gsdt:research-phase [X+1] ${GSD_WS}` — investigate unknowns
...
**Also available:**
- `/gsdt:discuss-phase [X+1] ${GSD_WS}` — revisit context
- `/gsdt:research-phase [X+1] ${GSD_WS}` — investigate unknowns
...
```bash
node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" config-set workflow._auto_chain_active false
```
...
`/gsdt:workstreams complete {current_ws_name} ${GSD_WS}`

See overall milestone progress:

`/gsdt:workstreams progress ${GSD_WS}`
...
Do NOT suggest `/gsdt:complete-milestone` or `/gsdt:new-milestone`.
...
Exit skill and invoke SlashCommand("/gsdt:complete-milestone {version} ${GSD_WS}")
...
`/gsdt:complete-milestone {version} ${GSD_WS}`
