...
<required_reading>
@~/.claude/gsdt/references/verification-patterns.md
@~/.claude/gsdt/templates/verification-report.md
</required_reading>
...
Use gsdt-tools to extract must_haves from each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" frontmatter get "$plan" --field must_haves)
...
```bash
PHASE_DATA=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" roadmap get-phase "${phase_number}" --raw)
```
...
Use gsdt-tools for artifact verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" verify artifacts "$plan")
...
Use gsdt-tools for key link verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" verify key-links "$plan")
...
See ~/.claude/gsdt/templates/verification-report.md for complete template.
</step>
