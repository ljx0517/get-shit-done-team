...
- File reference: `/gsdt:new-project --auto @prd.md`
...
Usage:
  /gsdt:new-project --auto @your-idea.md
  /gsdt:new-project --auto [paste or write your idea here]
...
```bash
mkdir -p .planning
node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" commit "chore: add project config" --files .planning/config.json
```
...
```bash
mkdir -p .planning
node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" commit "docs: initialize project" --files .planning/PROJECT.md
```
...
Spawn 4 parallel gsdt-project-researcher agents with path references:
...
```bash
node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" commit "docs: define v1 requirements" --files .planning/REQUIREMENTS.md
```
...
Spawn gsdt-roadmapper agent with path references:
...
- Re-spawn roadmapper with revision context:
  ...
  ", subagent_type="gsdt-roadmapper", model="{roadmapper_model}", description="Revise roadmap")
  ```
...
```bash
node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" commit "docs: create roadmap ([N] phases)" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md CLAUDE.md
```
...
Exit skill and invoke SlashCommand("/gsdt:discuss-phase 1 --auto")
...
```bash
PHASE1_SECTION=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" roadmap get-phase 1 2>/dev/null)
...
/gsdt:discuss-phase 1 — gather context and clarify approach
...
**Also available:**
- /gsdt:ui-phase 1 — generate UI design contract (recommended for frontend phases)
- /gsdt:plan-phase 1 — skip discussion, plan directly
...
- [ ] gsdt-roadmapper spawned with context
