...
**If auto mode:** Handle config before processing the idea document.
...
- Persist auto-chain flag, then continue:
...
AskUserQuestion([
  ...
  },
  {
    ...
    options: [
      ...
      { label: "Inherit", description: "Use the current session model for all agents (Vibe Agent Team / `/model`)" }
    ]
  }
])
```
