...
AGENT_SKILLS_ANALYZER=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" agent-skills gsdt-assumptions-analyzer 2>/dev/null)
```
...
Use /gsdt:progress to see available phases.
```
Exit workflow.
...
Spawn a `gsdt-assumptions-analyzer` agent to deeply analyze the codebase for this phase. This
...
```
Task(subagent_type="gsdt-assumptions-analyzer", prompt="""
...
```bash
node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" commit "docs(${padded_phase}): capture phase context (assumptions mode)" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
**Also available:**
- `/gsdt:plan-phase ${PHASE} --skip-research` — plan without research
- `/gsdt:ui-phase ${PHASE}` — generate UI design contract (if frontend work)
