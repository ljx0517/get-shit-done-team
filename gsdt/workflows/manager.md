...
```bash
INIT=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" init manager)
...
All {phase_count} phases done. Ready for final steps:
  → /gsdt:verify-work — run acceptance testing
  → /gsdt:complete-milestone — archive and wrap up
```
...
Steps:
1. Read the plan-phase workflow: cat ~/.claude/gsdt/workflows/plan-phase.md
2. Run: node \"$HOME/.claude/gsdt/bin/gsdt-tools.cjs\" init plan-phase {N}
...
5. Spawn a gsdt-planner subagent via Task() to create the plans.
6. If plan-checker is enabled, spawn a gsdt-plan-checker subagent to verify.
...
Steps:
1. Read the execute-phase workflow: cat ~/.claude/gsdt/workflows/execute-phase.md
2. Run: node \"$HOME/.claude/gsdt/bin/gsdt-tools.cjs\" init execute-phase {N}
3. Follow the workflow steps: discover plans, analyze dependencies, group into waves.
4. For each wave, spawn gsdt-executor subagents via Task() to execute plans in parallel.
5. After all waves complete, spawn a gsdt-verifier subagent if verifier is enabled.
