...
This workflow wires Phase 1 (session pipeline) and Phase 2 (profiling engine) into a cohesive user-facing experience. All heavy lifting is done by existing gsdt-tools.cjs subcommands and the gsdt-user-profiler agent -- this workflow orchestrates the sequence, handles branching, and provides the UX.
</purpose>
...
Key references:
- @$HOME/.claude/gsdt/references/ui-brand.md (display patterns)
- @$HOME/.claude/gsdt/agents/gsdt-user-profiler.md (profiler agent definition)
- @$HOME/.claude/gsdt/references/user-profiling.md (profiling reference doc)
</required_reading>
...
```bash
PROFILE_PATH="$HOME/.claude/gsdt/USER-PROFILE.md"
...
```bash
cp "$HOME/.claude/gsdt/USER-PROFILE.md" "$HOME/.claude/gsdt/USER-PROFILE.backup.md"
```
...
✓ Stores profile at $HOME/.claude/gsdt/USER-PROFILE.md
...
- options:
  ...
  - "Not now" -- Display "No worries. Run /gsdt:profile-user when ready." and exit
...
Run session scan:
```bash
SCAN_RESULT=$(node $HOME/.claude/gsdt/bin/gsdt-tools.cjs scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(node $HOME/.claude/gsdt/bin/gsdt-tools.cjs profile-sample --json 2>/dev/null)
```
...
**Spawn gsdt-user-profiler agent using Task tool:**

Use the Task tool to spawn the `gsdt-user-profiler` agent. Provide it with:
- The sampled JSONL file path from profile-sample output
- The user-profiling reference doc at `$HOME/.claude/gsdt/references/user-profiling.md`
...
Reference: @$HOME/.claude/gsdt/references/user-profiling.md
...
**Get questions:**
```bash
QUESTIONS=$(node $HOME/.claude/gsdt/bin/gsdt-tools.cjs profile-questionnaire --json 2>/dev/null)
```
...
```bash
ANSWERS_PATH=$(mktemp /tmp/gsdt-profile-answers-XXXXXX.json)
```
...
```bash
ANALYSIS_RESULT=$(node $HOME/.claude/gsdt/bin/gsdt-tools.cjs profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
```bash
ANALYSIS_PATH=$(mktemp /tmp/gsdt-profile-analysis-XXXXXX.json)
```
...
```bash
node $HOME/.claude/gsdt/bin/gsdt-tools.cjs write-profile --input "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Profile written to $HOME/.claude/gsdt/USER-PROFILE.md"
...
- options (ALL pre-selected by default):
  - "/gsdt:dev-preferences command file" -- "Load your preferences in any session"
...
**If no artifacts selected:** Display "No artifacts generated. Your profile is saved at $HOME/.claude/gsdt/USER-PROFILE.md" and jump to step 10.
...
**For /gsdt:dev-preferences (if selected):**

```bash
node $HOME/.claude/gsdt/bin/gsdt-tools.cjs generate-dev-preferences --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Generated /gsdt:dev-preferences at $HOME/.claude/commands/gsdt/dev-preferences.md"

**For CLAUDE.md profile section (if selected):**

```bash
node $HOME/.claude/gsdt/bin/gsdt-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node $HOME/.claude/gsdt/bin/gsdt-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json 2>/dev/null
```

Display: "✓ Added profile section to $HOME/.claude/CLAUDE.md"

**Error handling:** If any gsdt-tools.cjs call fails, display the error message and use AskUserQuestion to offer "Retry" or "Skip this artifact". On retry, re-run the command. On skip, continue to next artifact.
...
```bash
BACKUP_PATH="$HOME/.claude/gsdt/USER-PROFILE.backup.md"
```
...
```
Artifacts:
  ✓ /gsdt:dev-preferences   $HOME/.claude/commands/gsdt/dev-preferences.md
...
- [ ] Artifacts generated sequentially via gsdt-tools.cjs subcommands
