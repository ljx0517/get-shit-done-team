<purpose>
Validate built features through conversational testing with persistent state. Creates UAT.md that tracks test progress, survives /clear, and feeds gaps into /gsdt:plan-phase --gaps.
...
```bash
INIT=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" init verify-work "${PHASE_ARG}")
...
Provide a phase number to start testing (e.g., /gsdt:verify-work 4)
```
...
```bash
CHECKPOINT=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" uat render-checkpoint --file "$uat_path" --raw)
...
- `/gsdt:plan-phase {next}` — Plan next phase
- `/gsdt:execute-phase {next}` — Execute next phase
- `/gsdt:ui-review {phase}` — visual quality audit (if frontend files were modified)
```
</step>
...
- Follow @~/.claude/gsdt/workflows/diagnose-issues.md
...
Spawn gsdt-planner in --gaps mode:
...
<downstream_consumer>
Output consumed by /gsdt:execute-phase
...
Spawn gsdt-planner with revision context:
...
3. Abandon (exit, user runs /gsdt:plan-phase manually)
...
`/clear` then `/gsdt:execute-phase {phase} --gaps-only`
...
- [ ] If issues: gsdt-planner creates fix plans (gap_closure mode)
- [ ] If issues: gsdt-plan-checker verifies fix plans
- [ ] If issues: revision loop until plans pass (max 3 iterations)
- [ ] Ready for `/gsdt:execute-phase --gaps-only` when complete
</success_criteria>
