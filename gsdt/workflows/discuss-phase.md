...
1. **gsdt-phase-researcher** — Reads CONTEXT.md to know WHAT to research
   - "User wants card-based layout" → researcher investigates card component patterns
   - "Infinite scroll decided" → researcher looks into virtualization libraries

2. **gsdt-planner** — Reads CONTEXT.md to know WHAT decisions are locked
...
Enable text mode:
- Per-session: pass `--text` flag to any command (e.g., `/gsdt:discuss-phase --text`)
- Per-project: `gsdt-tools config-set workflow.text_mode true`
...
**Express path available:** If you already have a PRD or acceptance criteria document, use `/gsdt:plan-phase {phase} --prd path/to/prd.md` to skip this discussion and go straight to planning.
...
Use /gsdt:progress ${GSD_WS} to see available phases.
```
Exit workflow.
...
- options:
  - "Continue and replan after" — Capture context, then run /gsdt:plan-phase {X} ${GSD_WS} to replan
...
```bash
TODO_MATCHES=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" todo match-phase "${PHASE_NUMBER}")
```
...
3. Resolve model for advisor agents:
   ```bash
   ADVISOR_MODEL=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" resolve-model gsdt-advisor-researcher --raw)
   ```
...
2. For EACH user-selected gray area, spawn a Task() in parallel:
   ...
   Task(
     prompt="First, read @~/.claude/agents/gsdt-advisor-researcher.md for your role and instructions.
...
**Also available:**
- `/gsdt:plan-phase ${PHASE} --skip-research ${GSD_WS}` — plan without research
- `/gsdt:ui-phase ${PHASE} ${GSD_WS}` — generate UI design contract before planning (if phase has frontend work)
...
```bash
node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
```bash
node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" config-set workflow._auto_chain_active true
```
...
- **PLANNING COMPLETE** → Planning done, execution didn't complete:
  ...
  Continue: /gsdt:execute-phase ${PHASE} ${GSD_WS}
  ```
- **PLANNING INCONCLUSIVE / CHECKPOINT** → Stop chain:
  ```
  Auto-advance stopped: Planning needs input.
  Continue: /gsdt:plan-phase ${PHASE} ${GSD_WS}
  ```
- **GAPS FOUND** → Stop chain:
  ```
  Auto-advance stopped: Gaps found during execution.
  Continue: /gsdt:plan-phase ${PHASE} --gaps ${GSD_WS}
  ```
