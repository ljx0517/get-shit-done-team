<purpose>
Create executable phase prompts (PLAN.md files) for a roadmap phase with integrated research and verification. Default flow: Research (if needed) -> Plan -> Verify -> Done. Orchestrates gsdt-phase-researcher, gsdt-planner, and gsdt-plan-checker agents with a revision loop (max 3 iterations).
</purpose>
...
```bash
INIT=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" init plan-phase "$PHASE")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_RESEARCHER=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" agent-skills gsdt-phase-researcher 2>/dev/null)
AGENT_SKILLS_PLANNER=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" agent-skills gsdt-planner 2>/dev/null)
AGENT_SKILLS_CHECKER=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" agent-skills gsdt-plan-checker 2>/dev/null)
```
...
**If `planning_exists` is false:** Error — run `/gsdt:new-project` first.
...
Then re-run /gsdt:plan-phase {N} --reviews
```
Exit workflow.
...
```bash
PHASE_INFO=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" roadmap get-phase "${PHASE}")
```
...
5. Commit:
```bash
node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
If "Run discuss-phase first":
  **IMPORTANT:** Do NOT invoke discuss-phase as a nested Skill/Task call — AskUserQuestion
  ...
  ```
  Run this command first, then re-run /gsdt:plan-phase {X} ${GSD_WS}:

  /gsdt:discuss-phase {X} ${GSD_WS}
  ```
...
### Spawn gsdt-phase-researcher

```bash
PHASE_DESC=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" roadmap get-phase "${PHASE}" --pick section)
```
...
<files_to_read>
- {context_path} (USER DECISIONS from /gsdt:discuss-phase)
...
**If found:**
1. Read template: `~/.claude/gsdt/templates/VALIDATION.md`
...
UI_GATE_CFG=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" config-get workflow.ui_safety_gate 2>/dev/null || echo "true")
```
...
```bash
PHASE_SECTION=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" roadmap get-phase "${PHASE}" 2>/dev/null)
...
1. Generate UI-SPEC first — Run /gsdt:ui-phase {N} then re-run /gsdt:plan-phase {N}
...
- options:
  - "Generate UI-SPEC first" → Display: "Run `/gsdt:ui-phase {N} ${GSD_WS}` then re-run `/gsdt:plan-phase {N} ${GSD_WS}`". Exit workflow.
...
1. Re-run: `/gsdt:plan-phase {PHASE} --research ${GSD_WS}`
2. Disable Nyquist with the exact command:
   `node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" config-set workflow.nyquist_validation false`
3. Continue anyway (plans fail Dimension 8)

Proceed to Step 8 only if user selects 2 or 3.

## 8. Spawn gsdt-planner Agent
...
<downstream_consumer>
Output consumed by /gsdt:execute-phase. Plans need:
...
## 10. Spawn gsdt-plan-checker Agent
...
Task(
  ...
  subagent_type="gsdt-plan-checker",
  model="{checker_model}",
...
5. **Retry:** Restart Claude Code and run `/gsdt:plan-phase` again

If freezes persist, try `--skip-research` to reduce the agent chain from 3 to 2 agents:
```
/gsdt:plan-phase N --skip-research
```
</windows_troubleshooting>
...
- [ ] gsdt-phase-researcher spawned with CONTEXT.md
- [ ] Existing plans checked
- [ ] gsdt-planner spawned with CONTEXT.md + RESEARCH.md
- [ ] Plans created (PLANNING COMPLETE or CHECKPOINT handled)
- [ ] gsdt-plan-checker spawned with CONTEXT.md
