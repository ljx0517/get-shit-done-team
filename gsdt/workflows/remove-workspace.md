...
```bash
INIT=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" init remove-workspace "$WORKSPACE_NAME")
...
First run `/gsdt:list-workspaces` to show available workspaces, then ask:
