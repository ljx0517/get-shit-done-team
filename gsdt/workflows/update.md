...
for entry in "${ORDERED_RUNTIME_DIRS[@]}"; do
  runtime="${entry%%:*}"
  dir="${entry#*:}"
  if [ -f "./$dir/gsdt/VERSION" ] || [ -f "./$dir/gsdt/workflows/update.md" ]; then
    LOCAL_RUNTIME="$runtime"
    LOCAL_VERSION_FILE="./$dir/gsdt/VERSION"
    LOCAL_MARKER_FILE="./$dir/gsdt/workflows/update.md"
...
for entry in "${ORDERED_RUNTIME_DIRS[@]}"; do
  runtime="${entry%%:*}"
  dir="${entry#*:}"
  if [ -f "$HOME/$dir/gsdt/VERSION" ] || [ -f "$HOME/$dir/gsdt/workflows/update.md" ]; then
    GLOBAL_RUNTIME="$runtime"
    GLOBAL_VERSION_FILE="$HOME/$dir/gsdt/VERSION"
    GLOBAL_MARKER_FILE="$HOME/$dir/gsdt/workflows/update.md"
...
```bash
npm view gsdt version 2>/dev/null
```
...
To update manually: `npx gsdt --global`
```
...
**If LOCAL install:**
```bash
npx -y gsdt@latest "$RUNTIME_FLAG" --local
```

**If GLOBAL install:**
```bash
npx -y gsdt@latest "$RUNTIME_FLAG" --global
```

**If UNKNOWN install:**
```bash
npx -y gsdt@latest --claude --global
```
...
The SessionStart hook (`gsdt-check-update.js`) writes to the detected runtime's cache directory, so all paths must be cleared to prevent stale update indicators.
</step>
...
[View full changelog](https://github.com/gsd-build/gsdt/blob/main/CHANGELOG.md)
```
</step>
...
Check for gsdt-local-patches/backup-meta.json in the config directory.

**If patches found:**

```
Local patches were backed up before the update.
Run /gsdt:reapply-patches to merge your modifications into the new version.
```
