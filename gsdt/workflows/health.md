...
```bash
node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" validate health $REPAIR_FLAG
```
...
- [E001] config.json: JSON parse error at line 5
  Fix: Run /gsdt:health --repair to reset to defaults

- [E002] PROJECT.md not found
  Fix: Run /gsdt:new-project to create
```
...
```
---
N issues can be auto-repaired. Run: /gsdt:health --repair
```
</step>
...
```
Would you like to run /gsdt:health --repair to fix N issues automatically?
```
...
```bash
node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" validate health
```
