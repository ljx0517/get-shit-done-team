<purpose>
Create all phases necessary to close gaps identified by `/gsdt:audit-milestone`. Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase. One command creates all fix phases — no manual `/gsdt:add-phase` per gap.
</purpose>
...
(ls -t .gsdt-planning/v*-MILESTONE-AUDIT.md 2>/dev/null || true) | head -1
```
...
```
No audit gaps found. Run `/gsdt:audit-milestone` first.
```
...
HIGHEST=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" phases list --pick directories[-1])
```
...
grep -c "Pending" .gsdt-planning/REQUIREMENTS.md
```

## 8. Create Phase Directories

```bash
mkdir -p ".gsdt-planning/phases/{NN}-{name}"
```

## 9. Commit Roadmap and Requirements Update

```bash
node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" commit "docs(roadmap): add gap closure phases {N}-{M}" --files .gsdt-planning/ROADMAP.md .gsdt-planning/REQUIREMENTS.md
```
...
- `cat .gsdt-planning/ROADMAP.md` — see updated roadmap

---

**After all gap phases complete:**

`/gsdt:audit-milestone` — re-audit to verify gaps closed
`/gsdt:complete-milestone {version}` — archive when audit passes
```
...
- [ ] User knows to run `/gsdt:plan-phase` next
</success_criteria>
