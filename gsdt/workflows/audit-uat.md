...
```bash
AUDIT=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" audit-uat --raw)
```
...
1. **Close stale items:** `/gsdt:verify-work {phase}` — mark stale tests as resolved
2. **Run active tests:** Human UAT test plan below
3. **When prerequisites met:** Retest blocked items with `/gsdt:verify-work {phase}`
```
</step>
