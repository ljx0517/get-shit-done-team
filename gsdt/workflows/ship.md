...
```bash
CONFIG=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" state load)
```
...
```bash
node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" state update "Last Activity" "$(date +%Y-%m-%d)"
node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" state update "Status" "Phase ${PHASE_NUMBER} shipped — PR #${PR_NUMBER}"
```
...
- /gsdt:complete-milestone (if last phase in milestone)
- /gsdt:progress (to see what's next)
...
- /gsdt:complete-milestone — if all phases in milestone are done
- /gsdt:progress — see overall project state
- /gsdt:execute-phase {next} — continue to next phase
</offer_next>
