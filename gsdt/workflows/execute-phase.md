...
- **Claude Code:** Uses `Task(subagent_type="gsdt-executor", ...)` — blocks until complete, returns result
...
These are the valid GSDT subagent types registered in .claude/agents/ (or equivalent for your runtime).
...
- gsdt-verifier — Verifies phase completion, checks quality gates
...
```bash
INIT=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" init execute-phase "${PHASE_ARG}")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" agent-skills gsdt-executor 2>/dev/null)
```
...
Check if the current runtime is Copilot by testing for the `@gsdt-executor` agent pattern
...
if [[ ! "$ARGUMENTS" =~ --auto ]]; then
  node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" config-set workflow._auto_chain_active false 2>/dev/null
fi
```
</step>
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `~/.claude/gsdt/workflows/execute-plan.md` **inline**
...
- Maintains GSDT's planning/tracking structure
- Best for: small phases, bug fixes, verification gaps, learning GSDT
...
```bash
node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" state begin-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
```bash
PLAN_INDEX=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" phase-plan-index "${PHASE_NUMBER}")
```
...
2. **Spawn executor agents:**
   ...
   Task(
     ...
     prompt="
       ...
       For gsdt-tools commits: add --no-verify flag.
       For direct git commits: use git commit --no-verify -m "..."
       </parallel_execution>

       <execution_context>
       @~/.claude/gsdt/workflows/execute-plan.md
       @~/.claude/gsdt/templates/summary.md
       @~/.claude/gsdt/references/checkpoints.md
       @~/.claude/gsdt/references/tdd.md
       </execution_context>
...
5. **Handle failures:**
   ...
   **Known Claude Code bug (classifyHandoffIfNeeded):** If an agent reports "failed" with error containing `classifyHandoffIfNeeded is not defined`, this is a Claude Code runtime bug — not a GSDT or agent issue. The error fires in the completion handler AFTER all tool calls finish. In this case: run the same spot-checks as step 4 (SUMMARY.md exists, git commits present, no Self-Check: FAILED). If spot-checks PASS → treat as **successful**. If spot-checks FAIL → treat as real failure below.
...
5b. **Pre-wave dependency check (waves 2+ only):**
    ...
    ```bash
    node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" verify key-links {phase_dir}/{plan}-PLAN.md
    ```
...
```bash
AUTO_CHAIN=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" config-get workflow._auto_chain_active 2>/dev/null || echo "false")
AUTO_CFG=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" config-get workflow.auto_advance 2>/dev/null || echo "false")
```
...
```bash
POST_PLAN_INDEX=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" phase-plan-index "${PHASE_NUMBER}")
```
...
/gsdt:execute-phase {phase} ${GSD_WS}                # Continue remaining waves
/gsdt:execute-phase {phase} --wave {next} ${GSD_WS}  # Run the next wave explicitly
```
...
```bash
PARENT_INFO=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" find-phase "${PARENT_PHASE}" --raw)
...
```bash
VERIFIER_SKILLS=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" agent-skills gsdt-verifier 2>/dev/null)
```
...
Commit the file:
```bash
node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" commit "test({phase_num}): persist human verification items as UAT" --files "{phase_dir}/{phase_num}-HUMAN-UAT.md"
```
...
Items saved to `{phase_num}-HUMAN-UAT.md` — they will appear in `/gsdt:progress` and `/gsdt:audit-uat`.

"approved" → continue | Report issues → gap closure
```

**If user says "approved":** Proceed to `update_roadmap`. The HUMAN-UAT.md file persists with `status: partial` and will surface in future progress checks until the user runs `/gsdt:verify-work` on it.
...
Also: `/gsdt:verify-work {X} ${GSD_WS}` — manual testing first
```
...
```bash
COMPLETION=$(node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" phase complete "${PHASE_NUMBER}")
```
...
These items are tracked and will appear in `/gsdt:progress` and `/gsdt:audit-uat`.
```
...
**Exception:** If `gaps_found`, the `verify_phase_goal` step already presents the gap-closure path (`/gsdt:plan-phase {X} --gaps`). No additional routing needed — skip auto-advance.
...
Read and follow `~/.claude/gsdt/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.

**If none of `--auto`, `AUTO_CHAIN`, or `AUTO_CFG` is true:**

**STOP. Do not auto-advance. Do not execute transition. Do not plan next phase. Present options to the user and wait.**

**IMPORTANT: There is NO `/gsdt:transition` command. Never suggest it. The transition workflow is internal only.**

```
## ✓ Phase {X}: {Name} Complete

/gsdt:progress ${GSD_WS} — see updated roadmap
/gsdt:discuss-phase {next} ${GSD_WS} — discuss next phase before planning
/gsdt:plan-phase {next} ${GSD_WS} — plan next phase
/gsdt:execute-phase {next} ${GSD_WS} — execute next phase
```
...
<failure_handling>
- **classifyHandoffIfNeeded false failure:** Agent reports "failed" but error is `classifyHandoffIfNeeded is not defined` → Claude Code bug, not GSDT. Spot-check (SUMMARY exists, commits present) → if pass, treat as success
...
<resumption>
Re-run `/gsdt:execute-phase {phase}` → discover_plans finds completed SUMMARYs → skips them → resumes from first incomplete plan → continues wave execution.
