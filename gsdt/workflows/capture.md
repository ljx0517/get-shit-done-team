...
```bash
node ~/.claude/gsdt/bin/gsdt-tools.cjs verify-path-exists "$(pwd)/.claude/.gsdt-planning/ROADMAP.md"
```
...
node ~/.claude/gsdt/bin/gsdt-tools.cjs commit "chore: add project config" --files .claude/.gsdt-planning/config.json
...
将合并后的内容写入：
`.claude/.gsdt-planning/captures/idea.md`

然后**必须**直接触发：
```
Skill(skill="gsdt:new-project", args="--auto @.claude/.gsdt-planning/captures/idea.md")
```
