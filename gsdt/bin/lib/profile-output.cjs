...
const os = require('os');
const { output, error, safeReadFile, planningRoot } = require('./core.cjs');
...
const CLAUDE_MD_FALLBACKS = {
  project: 'Project not yet initialized. Run /gsdt:new-project to set up.',
...
const CLAUDE_MD_WORKFLOW_ENFORCEMENT = [
  ...
  '- `/gsdt:quick` for small fixes, doc updates, and ad-hoc tasks',
  '- `/gsdt:debug` for investigation and bug fixing',
  '- `/gsdt:execute-phase` for planned phase work',
  '',
...
const CLAUDE_MD_PROFILE_PLACEHOLDER = [
  ...
  '## Developer Profile',
  '',
  '> Profile not yet configured. Run `/gsdt:profile-user` to generate your developer profile.',
...
function generateStackSection(cwd) {
  const codebasePath = path.join(planningRoot(cwd), 'codebase', 'STACK.md');
  const researchPath = path.join(planningRoot(cwd), 'research', 'STACK.md');
...
function generateConventionsSection(cwd) {
  const conventionsPath = path.join(planningRoot(cwd), 'codebase', 'CONVENTIONS.md');
...
function generateArchitectureSection(cwd) {
  const architecturePath = path.join(planningRoot(cwd), 'codebase', 'ARCHITECTURE.md');
...
function cmdWriteProfile(cwd, options, raw) {
  ...
  if (!outputPath) {
    outputPath = path.join(os.homedir(), '.claude', 'gsdt', 'USER-PROFILE.md');
...
function cmdGenerateDevPreferences(cwd, options, raw) {
  ...
  if (analysis.data_source === 'questionnaire') {
    stackBlock = 'Stack preferences not available (questionnaire-only profile). Run `/gsdt:profile-user --refresh` with session data to populate.';
  ...
  let outputPath = options.output;
  if (!outputPath) {
    outputPath = path.join(os.homedir(), '.claude', 'commands', 'gsdt', 'dev-preferences.md');
  } else if (!path.isAbsolute(outputPath)) {
    outputPath = path.join(cwd, outputPath);
  }
  ...
  const result = {
    ...
    command_name: '/gsdt:dev-preferences',
...
function cmdGenerateClaudeProfile(cwd, options, raw) {
  ...
  const sectionLines = [
    ...
    '## Developer Profile',
    '',
    `> Generated by GSD from ${dataSource}. Run \`/gsdt:profile-user --refresh\` to update.`,
    '',
...
function cmdGenerateClaudeMd(cwd, options, raw) {
  ...
  if (profileStatus === 'placeholder_added') message += ' Run /gsdt:profile-user to unlock Developer Profile.';
