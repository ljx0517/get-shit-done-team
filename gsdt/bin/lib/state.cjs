...
function buildStateFrontmatter(bodyContent, cwd) {
  ...
  const fm = { gsdt_state_version: '1.0' };
 ...
 * File is written to .planning/WAITING.json (or .gsdt/WAITING.json if .gsdt exists).
 * Fixes #1034.
 */
function cmdSignalWaiting(cwd, type, question, options, phase, raw) {
  const gsdDir = fs.existsSync(path.join(cwd, '.gsdt')) ? path.join(cwd, '.gsdt') : planningDir(cwd);
...
function cmdSignalResume(cwd, raw) {
  const paths = [
    path.join(cwd, '.gsdt', 'WAITING.json'),
