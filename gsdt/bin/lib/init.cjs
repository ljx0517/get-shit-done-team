...
function cmdInitExecutePhase(cwd, phase, raw) {
  ...
  const result = {
    // Models
    executor_model: resolveModelInternal(cwd, 'gsdt-executor'),
    verifier_model: resolveModelInternal(cwd, 'gsdt-verifier'),
...
function cmdInitPlanPhase(cwd, phase, raw) {
  ...
  const result = {
    // Models
    researcher_model: resolveModelInternal(cwd, 'gsdt-phase-researcher'),
    planner_model: resolveModelInternal(cwd, 'gsdt-planner'),
    checker_model: resolveModelInternal(cwd, 'gsdt-plan-checker'),
...
function cmdInitNewProject(cwd, raw) {
  ...
  try {
    ...
    const skipDirs = new Set(['node_modules', '.git', '.claude', '__pycache__', 'target', 'dist', 'build']);
  ...
  const result = {
    // Models
    researcher_model: resolveModelInternal(cwd, 'gsdt-project-researcher'),
    synthesizer_model: resolveModelInternal(cwd, 'gsdt-research-synthesizer'),
    roadmapper_model: resolveModelInternal(cwd, 'gsdt-roadmapper'),
...
function cmdInitMapCodebase(cwd, raw) {
  ...
  const result = {
    // Models
    mapper_model: resolveModelInternal(cwd, 'gsdt-codebase-mapper'),
...
function cmdInitManager(cwd, raw) {
  ...
  if (!fs.existsSync(paths.roadmap)) {
    error('No ROADMAP.md found. Run /gsdt:new-milestone first.');
  }
  if (!fs.existsSync(paths.state)) {
    error('No STATE.md found. Run /gsdt:new-milestone first.');
  }
  ...
  for (const phase of phases) {
    ...
    if (phase.disk_status === 'planned' && phase.deps_satisfied) {
      recommendedActions.push({
        ...
        command: `/gsdt:execute-phase ${phase.number}`,
      });
      ...
      recommendedActions.push({
        ...
        command: `/gsdt:plan-phase ${phase.number}`,
      });
      ...
      recommendedActions.push({
        ...
        command: `/gsdt:discuss-phase ${phase.number}`,
      });
    }
  }
...
function cmdInitNewWorkspace(cwd, raw) {
  ...
  const defaultBase = path.join(homedir, 'gsdt-workspaces');
 ...
 * @param {string} agentType - The agent type (e.g., 'gsdt-executor', 'gsdt-planner')
 ...
 * Used by workflows: SKILLS=$(node "$TOOLS" agent-skills gsdt-executor 2>/dev/null)
 */
