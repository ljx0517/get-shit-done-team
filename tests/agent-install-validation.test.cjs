...
const MODEL_PROFILES = require('../gsdt/bin/lib/model-profiles.cjs').MODEL_PROFILES;
 ...
 * produces. gsdt-tools.cjs lives at <configDir>/gsdt/bin/gsdt-tools.cjs,
...
describe('init commands: agents_installed field (#1371)', () => {
  ...
  test('init execute-phase includes agents_installed=true when agents exist', () => {
    ...
    // gsdt-tools.cjs resolves agents from GSD_INSTALL_DIR or __dirname/../../agents
    const gsdInstallDir = path.resolve(__dirname, '..', 'gsdt', 'bin');
    const configDir = path.resolve(gsdInstallDir, '..', '..');
    const agentsDir = path.join(configDir, 'agents');

    // Agents already exist in the repo root /agents/ dir which is sibling to gsdt/
...
describe('validate health: agent installation check W010 (#1371)', () => {
  ...
  test('health check reports healthy when agents are installed (repo layout)', () => {
    // In the repo, agents/ exists as a sibling of gsdt/, so the
    // health check should find them via the gsdt-tools.cjs path resolution
