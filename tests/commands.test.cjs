...
`;
  ...
  test('multiple phases merged into single digest', () => {
    // Create phase 01
    const phase01Dir = path.join(tmpDir, '.gsdt-planning', 'phases', '01-foundation');
...
`
    ...
    // Create phase 02
    const phase02Dir = path.join(tmpDir, '.gsdt-planning', 'phases', '02-api');
...
describe('summary-extract command', () => {
  ...
  test('missing file returns error', () => {
    const result = runGsdTools('summary-extract .gsdt-planning/phases/01-test/01-01-SUMMARY.md', tmpDir);
...
`
    ...
    const result = runGsdTools('summary-extract .gsdt-planning/phases/01-foundation/01-01-SUMMARY.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);

    const output = JSON.parse(result.output);
    assert.strictEqual(output.path, '.gsdt-planning/phases/01-foundation/01-01-SUMMARY.md', 'path correct');
...
`
    ...
    const result = runGsdTools('summary-extract .gsdt-planning/phases/01-foundation/01-01-SUMMARY.md --fields one_liner,key_files,requirements_completed', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('todo complete command', () => {
  ...
  test('moves todo from pending to completed', () => {
    const pendingDir = path.join(tmpDir, '.gsdt-planning', 'todos', 'pending');
    ...
    // Verify moved
    assert.ok(
      !fs.existsSync(path.join(tmpDir, '.gsdt-planning', 'todos', 'pending', 'add-dark-mode.md')),
      'should be removed from pending'
    );
    assert.ok(
      fs.existsSync(path.join(tmpDir, '.gsdt-planning', 'todos', 'completed', 'add-dark-mode.md')),
      'should be in completed'
    );

    // Verify completion timestamp added
    const content = fs.readFileSync(
      path.join(tmpDir, '.gsdt-planning', 'todos', 'completed', 'add-dark-mode.md'),
      'utf-8'
    );
...
describe('todo match-phase command', () => {
  ...
  test('matches todo by keyword overlap with phase name', () => {
    ...
    fs.writeFileSync(path.join(tmpDir, '.gsdt-planning', 'ROADMAP.md'),
...
describe('scaffold command', () => {
  ...
  test('scaffolds context file', () => {
    ...
    // Verify file content
    const content = fs.readFileSync(
      path.join(tmpDir, '.gsdt-planning', 'phases', '03-api', '03-CONTEXT.md'),
      'utf-8'
    );
  ...
  test('scaffolds UAT file', () => {
    ...
    const content = fs.readFileSync(
      path.join(tmpDir, '.gsdt-planning', 'phases', '03-api', '03-UAT.md'),
      'utf-8'
    );
  ...
  test('scaffolds verification file', () => {
    ...
    const content = fs.readFileSync(
      path.join(tmpDir, '.gsdt-planning', 'phases', '03-api', '03-VERIFICATION.md'),
      'utf-8'
    );
  ...
  test('scaffolds phase directory', () => {
    ...
    assert.strictEqual(output.created, true);
    assert.ok(
      fs.existsSync(path.join(tmpDir, '.gsdt-planning', 'phases', '05-user-dashboard')),
...
describe('resolve-model command', () => {
  ...
  test('known agent returns model and profile without unknown_agent', () => {
    const result = runGsdTools('resolve-model gsdt-planner', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('commit command', () => {
  ...
  test('skips when .gsdt-planning is gitignored', () => {
    // Add .gsdt-planning/ to .gitignore and commit it so git recognizes the ignore
  ...
  test('creates real commit with correct hash', () => {
    // Create a new file in .gsdt-planning/
    fs.writeFileSync(path.join(tmpDir, '.gsdt-planning', 'test-file.md'), '# Test\n');

    const result = runGsdTools('commit "test: add test file" --files .gsdt-planning/test-file.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('amend mode works without crashing', () => {
    ...
    fs.writeFileSync(path.join(tmpDir, '.gsdt-planning', 'amend-file.md'), '# Initial\n');
    execSync('git add .gsdt-planning/amend-file.md', { cwd: tmpDir, stdio: 'pipe' });
    execSync('git commit -m "initial file"', { cwd: tmpDir, stdio: 'pipe' });

    // Modify the file and amend
    fs.writeFileSync(path.join(tmpDir, '.gsdt-planning', 'amend-file.md'), '# Amended\n');

    const result = runGsdTools('commit "ignored" --files .gsdt-planning/amend-file.md --amend', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('creates strategy branch before first commit when branching_strategy is milestone', () => {
    ...
    fs.writeFileSync(path.join(tmpDir, '.gsdt-planning', 'test-context.md'), '# Context\n');

    const result = runGsdTools('commit "docs: add context" --files .gsdt-planning/test-context.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('creates strategy branch before first commit when branching_strategy is phase', () => {
    ...
    fs.writeFileSync(path.join(tmpDir, '.gsdt-planning', 'phases', '01-setup', '01-CONTEXT.md'), '# Context\n');

    const result = runGsdTools(
      'commit "docs(01): add context" --files .gsdt-planning/phases/01-setup/01-CONTEXT.md',
      tmpDir
    );
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('websearch command', () => {
  const { cmdWebsearch } = require('../gsdt/bin/lib/commands.cjs');
  let origFetch;
  let origApiKey;
  let origWriteSync;
  let captured;
...
describe('stats command', () => {
  ...
  test('counts phases, plans, and summaries correctly', () => {
    ...
    const p2 = path.join(tmpDir, '.gsdt-planning', 'phases', '02-api');
    fs.mkdirSync(p1, { recursive: true });
...
`
  ...
  test('includes roadmap-only phases in totals and preserves hyphenated names', () => {
    const p1 = path.join(tmpDir, '.gsdt-planning', 'phases', '14-auth-hardening');
    const p2 = path.join(tmpDir, '.gsdt-planning', 'phases', '15-proof-generation');
    fs.mkdirSync(p1, { recursive: true });
    fs.mkdirSync(p2, { recursive: true });
