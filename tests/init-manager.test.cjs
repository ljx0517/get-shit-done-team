...
function writeRoadmap(tmpDir, phases) {
  ...
  fs.writeFileSync(path.join(tmpDir, '.gsdt-planning', 'ROADMAP.md'), content);
}

// Helper: write a minimal STATE.md
function writeState(tmpDir) {
  fs.writeFileSync(path.join(tmpDir, '.gsdt-planning', 'STATE.md'), '---\nstatus: active\n---\n# State\n');
}
...
function scaffoldPhase(tmpDir, num, opts = {}) {
  ...
  const dir = path.join(tmpDir, '.gsdt-planning', 'phases', `${padded}-${slug}`);
...
describe('init manager', () => {
  ...
  test('WAITING.json detected when present', () => {
    ...
    fs.writeFileSync(
      path.join(tmpDir, '.gsdt-planning', 'WAITING.json'),
