/**
 * Runtime Converter Tests — Vibe Agent Team + Gemini
 *
 ...
 *
 * Vibe Agent Team: convertClaudeToVibeAgentTeamFrontmatter (agent + command modes; opencode.json conventions)
 *   model: inherit is NOT added (runtime doesn't support it — see #1156)
 *   but mode: subagent IS added (required by agents in this layout).
...
describe('Vibe Agent Team agent conversion (isAgent: true)', () => {
  test('keeps name: field for agents', () => {
    const result = convertClaudeToVibeAgentTeamFrontmatter(SAMPLE_AGENT, { isAgent: true });
    const frontmatter = result.split('---')[1];
    assert.ok(frontmatter.includes('name: gsdt-executor'), 'name: should be preserved for agents');
  });

  test('does not add model: inherit (runtime does not support it)', () => {
    const result = convertClaudeToVibeAgentTeamFrontmatter(SAMPLE_AGENT, { isAgent: true });
    const frontmatter = result.split('---')[1];
    assert.ok(!frontmatter.includes('model: inherit'), 'model: inherit should NOT be added — runtime throws ProviderModelNotFoundError');
  });
...
Use $HOME/.claude/skills/ for reference.`;
    ...
    const result = convertClaudeToVibeAgentTeamFrontmatter(agentWithClaudePaths, { isAgent: true });
...
describe('Vibe Agent Team command conversion (isAgent: false, default)', () => {
  test('strips name: field for commands', () => {
    const result = convertClaudeToVibeAgentTeamFrontmatter(SAMPLE_COMMAND);
    const frontmatter = result.split('---')[1];
