...
} = require('../gsdt/bin/lib/model-profiles.cjs');
...
describe('MODEL_PROFILES', () => {
  test('contains all expected GSD agents', () => {
    const expectedAgents = [
      'gsdt-planner', 'gsdt-roadmapper', 'gsdt-executor',
      'gsdt-phase-researcher', 'gsdt-project-researcher', 'gsdt-research-synthesizer',
      'gsdt-debugger', 'gsdt-codebase-mapper', 'gsdt-verifier',
      'gsdt-plan-checker', 'gsdt-integration-checker', 'gsdt-nyquist-auditor',
      'gsdt-ui-researcher', 'gsdt-ui-checker', 'gsdt-ui-auditor',
    ];
...
describe('VALID_PROFILES', () => {
  ...
  test('is derived from MODEL_PROFILES keys', () => {
    const fromData = Object.keys(MODEL_PROFILES['gsdt-planner']);
...
describe('getAgentToModelMapForProfile', () => {
  test('returns correct models for balanced profile', () => {
    ...
    assert.strictEqual(map['gsdt-planner'], 'opus');
    assert.strictEqual(map['gsdt-codebase-mapper'], 'haiku');
    assert.strictEqual(map['gsdt-verifier'], 'sonnet');
  });

  test('returns correct models for budget profile', () => {
    const map = getAgentToModelMapForProfile('budget');
    assert.strictEqual(map['gsdt-planner'], 'sonnet');
    assert.strictEqual(map['gsdt-phase-researcher'], 'haiku');
  });

  test('returns correct models for quality profile', () => {
    const map = getAgentToModelMapForProfile('quality');
    assert.strictEqual(map['gsdt-planner'], 'opus');
    assert.strictEqual(map['gsdt-executor'], 'opus');
  });
...
describe('formatAgentToModelMapAsTable', () => {
  test('produces a table with header and separator', () => {
    const map = { 'gsdt-planner': 'opus', 'gsdt-executor': 'sonnet' };
    ...
    assert.ok(table.includes('gsdt-planner'), 'should list agent');
