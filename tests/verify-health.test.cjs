...
describe('validate health command', () => {
  ...
  test("returns 'broken' when .claude/.gsdt-planning directory is missing", () => {
    // createTempProject creates .claude/.gsdt-planning/phases — remove it entirely
    fs.rmSync(path.join(tmpDir, '.claude/.gsdt-planning'), { recursive: true, force: true });
  ...
  test('warns when PROJECT.md is missing', () => {
    // No PROJECT.md in .claude/.gsdt-planning
  ...
  test('warns about incorrectly named phase directories', () => {
    ...
    fs.mkdirSync(path.join(tmpDir, '.claude/.gsdt-planning', 'phases', 'bad_name'), { recursive: true });
  ...
  test('warns about phase on disk but not in ROADMAP', () => {
    ...
    fs.mkdirSync(path.join(tmpDir, '.claude/.gsdt-planning', 'phases', '99-orphan'), { recursive: true });
  ...
  test("returns 'healthy' when all checks pass", () => {
    ...
    const phaseDir = path.join(tmpDir, '.claude/.gsdt-planning', 'phases', '01-a');
    fs.mkdirSync(phaseDir, { recursive: true });
...
describe('validate health --repair command', () => {
  ...
  beforeEach(() => {
    ...
    // (E001, E003 are not repairable so we always need .claude/.gsdt-planning/ and ROADMAP.md)
    writeMinimalProjectMd(tmpDir);
    writeMinimalRoadmap(tmpDir, ['1']);
    fs.mkdirSync(path.join(tmpDir, '.claude/.gsdt-planning', 'phases', '01-a'), { recursive: true });
  });
  ...
  test('creates config.json with defaults when missing', () => {
    ...
    assert.strictEqual(diskConfig.phase_branch_template, 'gsdt/phase-{phase}-{slug}');
    assert.strictEqual(diskConfig.milestone_branch_template, 'gsdt/{milestone}-{slug}');
  });
  ...
  test('regenerates STATE.md when missing', () => {
    writeValidConfigJson(tmpDir);
    // No STATE.md
    const statePath = path.join(tmpDir, '.claude/.gsdt-planning', 'STATE.md');
