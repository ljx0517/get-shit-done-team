...
describe('validate health command', () => {
  ...
  // ─── Check 1: .gsdt-planning/ exists ───────────────────────────────────────────

  test("returns 'broken' when .gsdt-planning directory is missing", () => {
    // createTempProject creates .gsdt-planning/phases — remove it entirely
    fs.rmSync(path.join(tmpDir, '.gsdt-planning'), { recursive: true, force: true });
  ...
  test('warns when PROJECT.md is missing', () => {
    // No PROJECT.md in .gsdt-planning
  ...
  test('warns about incorrectly named phase directories', () => {
    ...
    fs.mkdirSync(path.join(tmpDir, '.gsdt-planning', 'phases', 'bad_name'), { recursive: true });
  ...
  test('warns about phase on disk but not in ROADMAP', () => {
    ...
    fs.mkdirSync(path.join(tmpDir, '.gsdt-planning', 'phases', '99-orphan'), { recursive: true });
  ...
  test("returns 'healthy' when all checks pass", () => {
    ...
    const phaseDir = path.join(tmpDir, '.gsdt-planning', 'phases', '01-a');
    fs.mkdirSync(phaseDir, { recursive: true });
...
describe('validate health --repair command', () => {
  ...
  beforeEach(() => {
    ...
    // (E001, E003 are not repairable so we always need .gsdt-planning/ and ROADMAP.md)
    writeMinimalProjectMd(tmpDir);
    writeMinimalRoadmap(tmpDir, ['1']);
    fs.mkdirSync(path.join(tmpDir, '.gsdt-planning', 'phases', '01-a'), { recursive: true });
  });
  ...
  test('regenerates STATE.md when missing', () => {
    writeValidConfigJson(tmpDir);
    // No STATE.md
    const statePath = path.join(tmpDir, '.gsdt-planning', 'STATE.md');
