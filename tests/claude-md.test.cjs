...
describe('generate-claude-md', () => {
  ...
  test('creates CLAUDE.md with workflow enforcement section', () => {
    ...
    assert.ok(content.includes('/gsdt:quick'));
    assert.ok(content.includes('/gsdt:debug'));
    assert.ok(content.includes('/gsdt:execute-phase'));
...
describe('new-project workflow includes CLAUDE.md generation', () => {
  ...
  test('new-project workflow generates CLAUDE.md before final commit', () => {
    ...
    assert.ok(content.includes('--files .gsdt-planning/ROADMAP.md .gsdt-planning/STATE.md .gsdt-planning/REQUIREMENTS.md CLAUDE.md'));
  });
