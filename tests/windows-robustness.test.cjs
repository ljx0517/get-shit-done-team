 ...
 * 4. findProjectRoot detects .git at same level as .gsdt-planning/
...
describe('workflow shell robustness', () => {
  ...
  test('glob loops in resume-project.md have existence guard', () => {
    ...
    for (const block of blocks) {
      // Look for `for ... in .gsdt-planning/` glob loops
      const forLoopMatch = block.code.match(/for\s+\w+\s+in\s+\.gsdt-planning\/[^;]+;\s*do/);
      if (forLoopMatch) {
...
describe('@file: handoff in workflows', () => {
  test('all workflows calling gsdt-tools init have @file: handler', () => {
    const workflowFiles = fs.readdirSync(WORKFLOWS_DIR)
      .filter(f => f.endsWith('.md'));
    ...
    for (const wf of workflowFiles) {
      ...
      // Check if this workflow calls gsdt-tools.cjs init
      if (/INIT=\$\(node.*gsdt-tools.*\binit\b/.test(content)) {
    ...
    assert.strictEqual(
      missing.length, 0,
      `Workflows calling gsdt-tools init without @file: handler (large output will be truncated):\n  ${missing.join('\n  ')}`
    );
  });
});
