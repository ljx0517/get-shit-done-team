...
const WORKFLOW_PATH = path.join(__dirname, '..', 'gsdt', 'workflows', 'autonomous.md');
...
describe('autonomous workflow ui-phase and ui-review integration (#1375)', () => {
  ...
  describe('step 3a.5 — UI design contract before planning', () => {
    ...
    test('UI design contract step invokes gsdt:ui-phase skill', () => {
      assert.ok(
        content.includes('skill="gsdt:ui-phase"'),
        'should invoke gsdt:ui-phase via Skill()'
      );
    });
  ...
  describe('step 3d.5 — UI review after execution', () => {
    ...
    test('UI review step invokes gsdt:ui-review skill', () => {
      assert.ok(
        content.includes('skill="gsdt:ui-review"'),
        'should invoke gsdt:ui-review via Skill()'
      );
    });
