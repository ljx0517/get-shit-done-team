...
// (no active /gsdt: command or Task subagent) and injects an advisory warning.
//
// This is a SOFT guard — it advises, not blocks. The edit still proceeds.
// The warning nudges Claude to use /gsdt:quick or /gsdt:fast instead of
...
process.stdin.on('end', () => {
  ...
  try {
    ...
    // Check if we're inside a GSD workflow (Task subagent or /gsdt: command)
    ...
    const output = {
      hookSpecificOutput: {
        ...
        additionalContext: `⚠️ WORKFLOW ADVISORY: You're editing ${path.basename(filePath)} directly without a GSD command. ` +
          ...
          'Consider using /gsdt:fast for trivial fixes or /gsdt:quick for larger changes ' +
