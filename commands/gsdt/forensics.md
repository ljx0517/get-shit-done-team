...
<objective>
Investigate what went wrong during a GSD workflow execution. Analyzes git history, `.gsdt-planning/` artifacts, and file system state to detect anomalies and generate a structured diagnostic report.

Purpose: Diagnose failed or stuck workflows so the user can understand root cause and take corrective action.
Output: Forensic report saved to `.gsdt-planning/forensics/`, presented inline, with optional issue creation.
</objective>

<execution_context>
@~/.claude/gsdt/workflows/forensics.md
</execution_context>
...
- `.gsdt-planning/STATE.md` (current position, session history)
- `.gsdt-planning/ROADMAP.md` (phase scope and progress)
- `.gsdt-planning/phases/*/` (PLAN.md, SUMMARY.md, VERIFICATION.md, CONTEXT.md)
- `.gsdt-planning/reports/SESSION_REPORT.md` (last session outcomes)
...
<process>
Read and execute the forensics workflow from @~/.claude/gsdt/workflows/forensics.md end-to-end.
</process>

<success_criteria>
- Evidence gathered from all available data sources
- At least 4 anomaly types checked (stuck loop, missing artifacts, abandoned work, crash/interruption)
- Structured forensic report written to `.gsdt-planning/forensics/report-{timestamp}.md`
