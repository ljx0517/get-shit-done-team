...
List all threads:
```bash
ls .gsdt-planning/threads/*.md 2>/dev/null
```
...
```
No threads found. Create one with: /gsdt:thread <description>
```
</mode_list>
...
```bash
cat ".gsdt-planning/threads/${THREAD_NAME}.md"
```
...
2. Create the threads directory if needed:
   ```bash
   mkdir -p .gsdt-planning/threads
   ```

3. Write the thread file:
   ```bash
   cat > ".gsdt-planning/threads/${SLUG}.md" << 'EOF'
   # Thread: {description}
...
5. Commit:
   ```bash
   node "$HOME/.claude/gsdt/bin/gsdt-tools.cjs" commit "docs: create thread — ${ARGUMENTS}" --files ".gsdt-planning/threads/${SLUG}.md"
   ```
...
6. Report:
   ...
   Thread: {slug}
   File: .gsdt-planning/threads/{slug}.md
...
- Lighter weight than /gsdt:pause-work — no phase state, no plan context
- The value is in Context and Next Steps — a cold-start session can pick up immediately
- Threads can be promoted to phases or backlog items when they mature:
  /gsdt:add-phase or /gsdt:add-backlog with context from the thread
- Thread files live in .gsdt-planning/threads/ — no collision with phases or other GSD structures
</notes>
