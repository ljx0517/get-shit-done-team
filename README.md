...
> [!NOTE]
> Codex installation uses skills (`skills/gsdt-*/SKILL.md`) rather than custom prompts.
...
# Codex (skills-first)
npx gsdt --codex --global    # Install to ~/.codex/
npx gsdt --codex --local     # Install to ./.codex/

# Copilot (GitHub Copilot CLI)
npx gsdt --copilot --global  # Install to ~/.github/
npx gsdt --copilot --local   # Install to ./.github/

# Cursor CLI
npx gsdt --cursor --global      # Install to ~/.cursor/
npx gsdt --cursor --local       # Install to ./.cursor/

# Windsurf (Codeium, VS Code-based)
npx gsdt --windsurf --global    # Install to ~/.windsurf/
npx gsdt --windsurf --local     # Install to ./.windsurf/

# Antigravity (Google, skills-first, Gemini-based)
npx gsdt --antigravity --global # Install to ~/.gemini/antigravity/
npx gsdt --antigravity --local  # Install to ./.agent/

# All runtimes
npx gsdt --all --global      # Install to all directories
```
...
> **Already have code?** Run `/gsdt:map-codebase` first. It spawns parallel agents to analyze your stack, architecture, conventions, and concerns. Then `/gsdt:new-project` knows your codebase — questions focus on what you're adding, and planning automatically loads your patterns.
...
> **Assumptions Mode:** Prefer codebase analysis over questions? Set `workflow.discuss_mode` to `assumptions` in `/gsdt:settings`. The system reads your code, surfaces what it would do and why, and only asks you to correct what's wrong. See [Discuss Mode](docs/workflow-discuss-mode.md).
...
If everything passes, you move on. If something's broken, you don't manually debug — you just run `/gsdt:execute-phase` again with the fix plans it created.
...
/gsdt:verify-work 2
/gsdt:ship 2                  # Create PR from verified work
...
...
If you want faster intake during discussion, use `/gsdt:discuss-phase <n> --batch` to answer a small grouped set of questions at once instead of one-by-one.

Each phase gets your input (discuss), proper research (plan), clean execution (execute), and human verification (verify). Context stays fresh. Quality stays high.

When all phases are done, `/gsdt:complete-milestone` archives the milestone and tags the release.

Then `/gsdt:new-milestone` starts the next version — same flow as `new-project` but for your existing codebase. You describe what you want to build next, the system researches the domain, you scope requirements, and it creates a fresh roadmap. Each milestone is a clean cycle: define → build → ship.
...
|---------|--------------|
| `/gsdt:new-project [--auto]` | Full initialization: questions → research → requirements → roadmap |
| `/gsdt:discuss-phase [N] [--auto] [--analyze]` | Capture implementation decisions before planning (`--analyze` adds trade-off analysis) |
| `/gsdt:plan-phase [N] [--auto] [--reviews]` | Research + plan + verify for a phase (`--reviews` loads codebase review findings) |
| `/gsdt:execute-phase <N>` | Execute all plans in parallel waves, verify when complete |
| `/gsdt:verify-work [N]` | Manual user acceptance testing ¹ |
| `/gsdt:ship [N] [--draft]` | Create PR from verified phase work with auto-generated body |
| `/gsdt:next` | Automatically advance to the next logical workflow step |
| `/gsdt:fast <text>` | Inline trivial tasks — skips planning entirely, executes immediately |
| `/gsdt:audit-milestone` | Verify milestone achieved its definition of done |
| `/gsdt:complete-milestone` | Archive milestone, tag release |
| `/gsdt:new-milestone [name]` | Start next version: questions → research → requirements → roadmap |
| `/gsdt:forensics [desc]` | Post-mortem investigation of failed workflow runs (diagnoses stuck loops, missing artifacts, git anomalies) |
| `/gsdt:milestone-summary [version]` | Generate comprehensive project summary for team onboarding and review |

### Workstreams

| Command | What it does |
|---------|--------------|
| `/gsdt:workstreams list` | Show all workstreams and their status |
| `/gsdt:workstreams create <name>` | Create a namespaced workstream for parallel milestone work |
| `/gsdt:workstreams switch <name>` | Switch active workstream |
| `/gsdt:workstreams complete <name>` | Complete and merge a workstream |
...
| Command | What it does |
|---------|--------------|
| `/gsdt:ui-phase [N]` | Generate UI design contract (UI-SPEC.md) for frontend phases |
| `/gsdt:ui-review [N]` | Retroactive 6-pillar visual audit of implemented frontend code |
...
| Command | What it does |
|---------|--------------|
| `/gsdt:map-codebase [area]` | Analyze existing codebase before new-project |

### Phase Management

| Command | What it does |
|---------|--------------|
| `/gsdt:add-phase` | Append phase to roadmap |
| `/gsdt:insert-phase [N]` | Insert urgent work between phases |
| `/gsdt:remove-phase [N]` | Remove future phase, renumber |
| `/gsdt:list-phase-assumptions [N]` | See Claude's intended approach before planning |
| `/gsdt:plan-milestone-gaps` | Create phases to close gaps from audit |

### Session

| Command | What it does |
|---------|--------------|
| `/gsdt:pause-work` | Create handoff when stopping mid-phase (writes HANDOFF.json) |
| `/gsdt:resume-work` | Restore from last session |
| `/gsdt:session-report` | Generate session summary with work performed and outcomes |

### Workstreams

| Command | What it does |
|---------|--------------|
| `/gsdt:workstreams` | Manage parallel workstreams (list, create, switch, status, progress, complete) |
...
| Command | What it does |
|---------|--------------|
| `/gsdt:plant-seed <idea>` | Capture forward-looking ideas with trigger conditions — surfaces at the right milestone |
| `/gsdt:add-backlog <desc>` | Add idea to backlog parking lot (999.x numbering, outside active sequence) |
| `/gsdt:review-backlog` | Review and promote backlog items to active milestone or remove stale entries |
| `/gsdt:thread [name]` | Persistent context threads — lightweight cross-session knowledge for work spanning multiple sessions |

### Utilities

| Command | What it does |
|---------|--------------|
| `/gsdt:settings` | Configure model profile and workflow agents |
| `/gsdt:set-profile <profile>` | Switch model profile (quality/balanced/budget/inherit) |
...
Use `/gsdt:settings` to toggle these, or override per-invocation:
...
- Verify files exist in `~/.claude/commands/gsdt/` (global) or `./.claude/commands/gsdt/` (local)
- For Codex, verify skills exist in `~/.codex/skills/gsdt-*/SKILL.md` (global) or `./.codex/skills/gsdt-*/SKILL.md` (local)

**Commands not working as expected?**
- Run `/gsdt:help` to verify installation
- Re-run `npx gsdt` to reinstall
...
npx gsdt --windsurf --global --uninstall
...
npx gsdt --windsurf --local --uninstall
