...
**Claude Code、Vibe Agent Team、Gemini CLI、Codex、Copilot、Antigravity向けの軽量かつ強力なメタプロンプティング、コンテキストエンジニアリング、仕様駆動開発システム。**
...
[![GitHub stars](https://img.shields.io/github/stars/gsd-build/get-shit-done-team?style=for-the-badge&logo=github&color=181717)](https://github.com/gsd-build/get-shit-done-team)
...
仕様駆動開発ツールは他にもあります（Speckit など）。しかしどれも必要以上に複雑にしているように見えます（スプリントセレモニー、ストーリーポイント、ステークホルダーとの同期、振り返り、Jiraワークフローなど）。あるいは、何を作ろうとしているのかの全体像を本当には理解していません。私は50人規模のソフトウェア会社ではありません。エンタープライズごっこをしたいわけではありません。ただ、うまく動く素晴らしいものを作りたいクリエイティブな人間です。
...
インストーラーが以下の選択を求めます：
1. **ランタイム** — Claude Code、Vibe Agent Team、Gemini、Codex、Copilot、Cursor、Antigravity、またはすべて（インタラクティブ複数選択 — 1回のインストールセッションで複数のランタイムを選択可能）
2. **インストール先** — グローバル（全プロジェクト）またはローカル（現在のプロジェクトのみ）

確認方法：
- Claude Code / Gemini: `/gsdt:help`
- Vibe Agent Team: `/gsdt-help`（既定は `~/.config/opencode/` にインストール）
- Codex: `$gsdt-help`
- Copilot: `/gsdt:help`
- Antigravity: `/gsdt:help`

> [!NOTE]
> Codexのインストールでは、カスタムプロンプトではなくスキル（`skills/gsdt-*/SKILL.md`）を使用します。
...
```bash
# Claude Code
npx gsdt --claude --global   # ~/.claude/ にインストール
npx gsdt --claude --local    # ./.claude/ にインストール

# Vibe Agent Team（flat な command/；設定は ~/.config/opencode/）
npx gsdt --vibe-agent-team --global
# エイリアス: npx gsdt --opencode --global

# Gemini CLI
npx gsdt --gemini --global   # ~/.gemini/ にインストール

# Codex（スキルファースト）
npx gsdt --codex --global    # ~/.codex/ にインストール
npx gsdt --codex --local     # ./.codex/ にインストール

# Copilot（GitHub Copilot CLI）
npx gsdt --copilot --global  # ~/.github/ にインストール
npx gsdt --copilot --local   # ./.github/ にインストール

# Cursor CLI
npx gsdt --cursor --global      # ~/.cursor/ にインストール
npx gsdt --cursor --local       # ./.cursor/ にインストール

# Antigravity（Google、スキルファースト、Geminiベース）
npx gsdt --antigravity --global # ~/.gemini/antigravity/ にインストール
npx gsdt --antigravity --local  # ./.agent/ にインストール

# 全ランタイム
npx gsdt --all --global      # すべてのディレクトリにインストール
```

`--global`（`-g`）または `--local`（`-l`）でインストール先の質問をスキップできます。
`--claude`、`--vibe-agent-team`（または `--opencode`）、`--gemini`、`--codex`、`--copilot`、`--cursor`、`--antigravity`、または `--all` でランタイムの質問をスキップできます。
...
> **既存のコードがある場合は？** まず `/gsdt:map-codebase` を実行してください。並列エージェントが起動し、スタック、アーキテクチャ、規約、懸念点を分析します。その後 `/gsdt:new-project` がコードベースを把握した状態で動作し、質問は追加する内容に焦点を当て、計画時にはパターンが自動的に読み込まれます。
...
**作成されるファイル：** `PROJECT.md`、`REQUIREMENTS.md`、`ROADMAP.md`、`STATE.md`、`.gsdt-planning/research/`
...
> **前提モード：** 質問よりもコードベース分析を優先したい場合は、`/gsdt:settings` で `workflow.discuss_mode` を `assumptions` に設定してください。システムがコードを読み、何をなぜそうするかを提示し、間違っている部分だけ修正を求めます。詳しくは[ディスカスモード](docs/ja-JP/workflow-discuss-mode.md)をご覧ください。
...
すべてパスすれば次に進みます。何か壊れていれば、手動でデバッグする必要はありません — 作成された修正プランで `/gsdt:execute-phase` を再度実行するだけです。
...
ディスカッション中のインプットを速くしたい場合は、`/gsdt:discuss-phase <n> --batch` で1つずつではなく小さなグループにまとめた質問に一括で回答できます。

各フェーズであなたのインプット（discuss）、適切なリサーチ（plan）、クリーンな実行（execute）、人間による検証（verify）が行われます。コンテキストは常にフレッシュ。品質は常に高い。

すべてのフェーズが完了したら、`/gsdt:complete-milestone` でマイルストーンをアーカイブしリリースをタグ付けします。

次に `/gsdt:new-milestone` で次のバージョンを開始します — `new-project` と同じフローですが既存のコードベース向けです。次に構築したいものを説明し、システムがドメインを調査し、要件をスコーピングし、新しいロードマップを作成します。各マイルストーンはクリーンなサイクルです：定義 → 構築 → シップ。
...
- **別トラッキング** — `.gsdt-planning/quick/` に保存、フェーズとは別管理
...
**作成されるファイル：** `.gsdt-planning/quick/001-add-dark-mode-toggle/PLAN.md`、`SUMMARY.md`
...
| コマンド | 説明 |
|---------|--------------|
| `/gsdt:new-project [--auto]` | フル初期化：質問 → リサーチ → 要件定義 → ロードマップ |
| `/gsdt:discuss-phase [N] [--auto] [--analyze]` | 計画前に実装の決定事項をキャプチャ（`--analyze` でトレードオフ分析を追加） |
| `/gsdt:plan-phase [N] [--auto] [--reviews]` | フェーズのリサーチ + プラン + 検証（`--reviews` でコードベースレビューの発見事項を読み込み） |
| `/gsdt:execute-phase <N>` | 全プランを並列ウェーブで実行し、完了時に検証 |
| `/gsdt:verify-work [N]` | 手動ユーザー受入テスト ¹ |
| `/gsdt:ship [N] [--draft]` | 検証済みのフェーズ作業から自動生成された本文付きのPRを作成 |
| `/gsdt:next` | 次の論理的なワークフローステップに自動的に進む |
| `/gsdt:fast <text>` | インラインの軽微タスク — 計画を完全にスキップし即座に実行 |
| `/gsdt:audit-milestone` | マイルストーンが完了の定義を達成したか検証 |
| `/gsdt:complete-milestone` | マイルストーンをアーカイブし、リリースをタグ付け |
| `/gsdt:new-milestone [name]` | 次のバージョンを開始：質問 → リサーチ → 要件定義 → ロードマップ |
| `/gsdt:forensics [desc]` | 失敗したワークフロー実行の事後分析（停止ループ、欠落成果物、git異常の診断） |
| `/gsdt:milestone-summary [version]` | チームオンボーディングとレビュー向けの包括的なプロジェクトサマリーを生成 |

### ワークストリーム

| コマンド | 説明 |
|---------|--------------|
| `/gsdt:workstreams list` | 全ワークストリームとそのステータスを表示 |
| `/gsdt:workstreams create <name>` | 並列マイルストーン作業用の名前空間付きワークストリームを作成 |
| `/gsdt:workstreams switch <name>` | アクティブなワークストリームを切り替え |
| `/gsdt:workstreams complete <name>` | ワークストリームを完了しマージ |

### マルチプロジェクトワークスペース

| コマンド | 説明 |
|---------|--------------|
| `/gsdt:new-workspace` | リポジトリのコピー（worktreeまたはクローン）で隔離されたワークスペースを作成 |
| `/gsdt:list-workspaces` | すべてのGSDワークスペースとそのステータスを表示 |
| `/gsdt:remove-workspace` | ワークスペースを削除しworktreeをクリーンアップ |

### UIデザイン

| コマンド | 説明 |
|---------|--------------|
| `/gsdt:ui-phase [N]` | フロントエンドフェーズ用のUIデザイン契約（UI-SPEC.md）を生成 |
| `/gsdt:ui-review [N]` | 実装済みフロントエンドコードの6つの柱によるビジュアル監査（遡及的） |
...
| コマンド | 説明 |
|---------|--------------|
| `/gsdt:map-codebase [area]` | new-project前に既存のコードベースを分析 |

### フェーズ管理

| コマンド | 説明 |
|---------|--------------|
| `/gsdt:add-phase` | ロードマップにフェーズを追加 |
| `/gsdt:insert-phase [N]` | フェーズ間に緊急作業を挿入 |
| `/gsdt:remove-phase [N]` | 将来のフェーズを削除し番号を振り直し |
| `/gsdt:list-phase-assumptions [N]` | 計画前にClaudeの意図するアプローチを確認 |
| `/gsdt:plan-milestone-gaps` | 監査で見つかったギャップを埋めるフェーズを作成 |

### セッション

| コマンド | 説明 |
|---------|--------------|
| `/gsdt:pause-work` | フェーズ途中で停止する際の引き継ぎを作成（HANDOFF.jsonを書き込み） |
| `/gsdt:resume-work` | 前回のセッションから復元 |
| `/gsdt:session-report` | 実行した作業と結果のセッションサマリーを生成 |

### ワークストリーム

| コマンド | 説明 |
|---------|--------------|
| `/gsdt:workstreams` | 並列ワークストリームを管理（list、create、switch、status、progress、complete） |
...
| `/gsdt:pr-branch` | `.gsdt-planning/` コミットをフィルタリングしたクリーンなPRブランチを作成 |
...
| コマンド | 説明 |
|---------|--------------|
| `/gsdt:plant-seed <idea>` | トリガー条件付きの将来志向のアイデアをキャプチャ — 適切なマイルストーンで浮上 |
| `/gsdt:add-backlog <desc>` | バックログのパーキングロットにアイデアを追加（999.xナンバリング、アクティブシーケンス外） |
| `/gsdt:review-backlog` | バックログ項目をレビューし、アクティブマイルストーンに昇格またはstaleエントリを削除 |
| `/gsdt:thread [name]` | 永続コンテキストスレッド — 複数セッションにまたがる作業用の軽量クロスセッション知識 |

### ユーティリティ

| コマンド | 説明 |
|---------|--------------|
| `/gsdt:settings` | モデルプロファイルとワークフローエージェントを設定 |
| `/gsdt:set-profile <profile>` | モデルプロファイルを切り替え（quality/balanced/budget/inherit） |
| `/gsdt:add-todo [desc]` | 後で取り組むアイデアをキャプチャ |
| `/gsdt:check-todos` | 保留中のtodoを一覧表示 |
| `/gsdt:debug [desc]` | 永続状態を持つ体系的デバッグ |
...
| `/gsdt:health [--repair]` | `.gsdt-planning/` ディレクトリの整合性を検証、`--repair` で自動修復 |
...
GSDはプロジェクト設定を `.gsdt-planning/config.json` に保存します。`/gsdt:new-project` 実行時に設定するか、後から `/gsdt:settings` で更新できます。完全な設定スキーマ、ワークフロートグル、gitブランチオプション、エージェントごとのモデル内訳については、[ユーザーガイド](docs/ja-JP/USER-GUIDE.md#configuration-reference)をご覧ください。
...
非Anthropicプロバイダー（OpenRouter、ローカルモデル）を使用する場合や、現在のランタイムのモデル選択に従う場合（例：Vibe Agent Team `/model`）は `inherit` を使用してください。

または `/gsdt:settings` で設定できます。
...
これらのトグルには `/gsdt:settings` を使用するか、呼び出し時にオーバーライドできます：
- `/gsdt:plan-phase --skip-research`
- `/gsdt:plan-phase --skip-verify`
...
| `planning.commit_docs` | `true` | `.gsdt-planning/` をgitで追跡 |
...
- **PreToolUseプロンプトガードフック** — `gsdt-prompt-guard` が `.gsdt-planning/` への書き込みに埋め込まれたインジェクションベクトルをスキャン（アドバイザリー、ブロッキングではない）
...
- `~/.claude/commands/gsdt/`（グローバル）または `./.claude/commands/gsdt/`（ローカル）にファイルが存在するか確認してください
- Codexの場合、`~/.codex/skills/gsdt-*/SKILL.md`（グローバル）または `./.codex/skills/gsdt-*/SKILL.md`（ローカル）にスキルが存在するか確認してください

**コマンドが期待通りに動作しない？**
- `/gsdt:help` を実行してインストールを確認してください
- `npx gsdt` を再実行して再インストールしてください
...
Vibe Agent Team、Gemini CLI、Codexは `npx gsdt` でネイティブサポートされています。

以下のコミュニティポートがマルチランタイムサポートの先駆けとなりました：

| プロジェクト | プラットフォーム | 説明 |
|---------|----------|-------------|
| [gsdt-opencode](https://github.com/rokicool/gsdt-opencode) | Vibe Agent Team | 初期コミュニティ版（リポジトリ名は歴史的） |
| gsdt-gemini（アーカイブ済み） | Gemini CLI | uberfuzzyによるオリジナルのGemini対応版 |

---

## スター履歴

<a href="https://star-history.com/#gsd-build/get-shit-done-team&Date">
 <picture>
   <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=gsd-build/get-shit-done-team&type=Date&theme=dark" />
   <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/svg?repos=gsd-build/get-shit-done-team&type=Date" />
   <img alt="Star History Chart" src="https://api.star-history.com/svg?repos=gsd-build/get-shit-done-team&type=Date" />
 </picture>
</a>
