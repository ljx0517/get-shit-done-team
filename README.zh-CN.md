...
所以我做了 GSDT。复杂性在系统内部，不在你的工作流里。幕后是上下文工程、XML 提示格式、子代理编排、状态管理；你看到的是几个真能工作的命令。
...
GSDT 解决的就是这个问题。它是让 Claude Code 变得可靠的上下文工程层。你只要描述想法，系统会自动提取它需要知道的一切，然后让 Claude Code 去干活。
...
> [!NOTE]
> Codex 安装走的是 skill 机制（`skills/gsdt-*/SKILL.md`），不是自定义 prompt。
...
# Codex（以 skills 为主）
npx gsdt --codex --global    # 安装到 ~/.codex/
npx gsdt --codex --local     # 安装到 ./.codex/

# Copilot（GitHub Copilot CLI）
npx gsdt --copilot --global  # 安装到 ~/.github/
npx gsdt --copilot --local   # 安装到 ./.github/

# Cursor CLI
npx gsdt --cursor --global   # 安装到 ~/.cursor/
npx gsdt --cursor --local    # 安装到 ./.cursor/

# Antigravity（Google，以 skills 为主，基于 Gemini）
npx gsdt --antigravity --global # 安装到 ~/.gemini/antigravity/
npx gsdt --antigravity --local  # 安装到 ./.agent/

# 所有运行时
npx gsdt --all --global      # 安装到所有目录
```
...
GSDT 的设计目标是无摩擦自动化。运行 Claude Code 时建议使用：
...
> [!TIP]
> 这才是 GSDT 的预期用法。连 `date` 和 `git commit` 都要来回确认 50 次，整个体验就废了。
...
> **已经有现成代码库？** 先运行 `/gsdt:map-codebase`。它会并行拉起多个代理分析你的技术栈、架构、约定和风险点。之后 `/gsdt:new-project` 就会真正“理解”你的代码库，提问会聚焦在你打算新增的部分，规划时也会自动加载你的现有模式。
...
如果一切通过，就进入下一步；如果哪里坏了，你不需要手动 debug，只要重新运行 `/gsdt:execute-phase`，执行它自动生成的修复计划即可。
...
```
/gsdt:next                    # 自动检测并执行下一步
```

循环执行 **讨论 → 规划 → 执行 → 验证 → 发布**，直到整个里程碑完成。

如果你希望在讨论阶段更快收集信息，可以用 `/gsdt:discuss-phase <n> --batch`，一次回答一小组问题，而不是逐个问答。

每个阶段都会得到你的输入（discuss）、充分研究（plan）、干净执行（execute）和人工验证（verify）。上下文始终保持新鲜，质量也能持续稳定。

当所有阶段完成后，`/gsdt:complete-milestone` 会归档当前里程碑并打 release tag。

接着用 `/gsdt:new-milestone` 开启下一个版本。它和 `new-project` 流程相同，只是面向你现有的代码库。你描述下一步想构建什么，系统研究领域、梳理需求，再产出新的路线图。每个里程碑都是一个干净周期：定义 → 构建 → 发布。
...
快速模式保留 GSDT 的核心保障（原子提交、状态跟踪），但路径更短：
...
| 命令 | 作用 |
|------|------|
| `/gsdt:new-project [--auto]` | 完整初始化：提问 → 研究 → 需求 → 路线图 |
| `/gsdt:discuss-phase [N] [--auto] [--analyze]` | 在规划前收集实现决策（`--analyze` 增加权衡分析） |
| `/gsdt:plan-phase [N] [--auto] [--reviews]` | 为某个阶段执行研究 + 规划 + 验证（`--reviews` 加载代码库审查结果） |
| `/gsdt:execute-phase <N>` | 以并行 wave 执行全部计划，完成后验证 |
| `/gsdt:verify-work [N]` | 人工用户验收测试 ¹ |
| `/gsdt:ship [N] [--draft]` | 从已验证的阶段工作创建 PR，自动生成 PR 描述 |
| `/gsdt:fast <text>` | 内联处理琐碎任务——完全跳过规划，立即执行 |
| `/gsdt:next` | 自动推进到下一个逻辑工作流步骤 |
| `/gsdt:audit-milestone` | 验证里程碑是否达到完成定义 |
| `/gsdt:complete-milestone` | 归档里程碑并打 release tag |
| `/gsdt:new-milestone [name]` | 开始下一个版本：提问 → 研究 → 需求 → 路线图 |
| `/gsdt:milestone-summary` | 从已完成的里程碑产物生成项目概览，用于团队上手 |
| `/gsdt:forensics` | 对失败或卡住的工作流进行事后调查 |

### 工作流（Workstreams）

| 命令 | 作用 |
|------|------|
| `/gsdt:workstreams list` | 显示所有工作流及其状态 |
| `/gsdt:workstreams create <name>` | 创建命名空间工作流，用于并行里程碑工作 |
| `/gsdt:workstreams switch <name>` | 切换当前活跃工作流 |
| `/gsdt:workstreams complete <name>` | 完成并合并工作流 |

### 多项目工作区

| 命令 | 作用 |
|------|------|
| `/gsdt:new-workspace` | 创建隔离工作区，包含仓库副本（worktree 或 clone） |
| `/gsdt:list-workspaces` | 显示所有 GSDT 工作区及其状态 |
| `/gsdt:remove-workspace` | 移除工作区并清理 worktree |

### UI 设计

| 命令 | 作用 |
|------|------|
| `/gsdt:ui-phase [N]` | 为前端阶段生成 UI 设计合约（UI-SPEC.md） |
| `/gsdt:ui-review [N]` | 对已实现前端代码进行 6 维视觉审计 |

### 导航

| 命令 | 作用 |
|------|------|
| `/gsdt:progress` | 我现在在哪？下一步是什么？ |
| `/gsdt:next` | 自动检测状态并执行下一步 |
| `/gsdt:help` | 显示全部命令和使用指南 |
| `/gsdt:update` | 更新 GSDT，并预览变更日志 |
| `/gsdt:join-discord` | 加入 GSDT Discord 社区 |

### Brownfield

| 命令 | 作用 |
|------|------|
| `/gsdt:map-codebase` | 在 `new-project` 前分析现有代码库 |

### 阶段管理

| 命令 | 作用 |
|------|------|
| `/gsdt:add-phase` | 在路线图末尾追加 phase |
| `/gsdt:insert-phase [N]` | 在 phase 之间插入紧急工作 |
| `/gsdt:remove-phase [N]` | 删除未来 phase，并重编号 |
| `/gsdt:list-phase-assumptions [N]` | 在规划前查看 Claude 打算采用的方案 |
| `/gsdt:plan-milestone-gaps` | 为 audit 发现的缺口创建 phase |
...
| 命令 | 作用 |
|------|------|
| `/gsdt:plant-seed <idea>` | 将想法存入积压停车场，留待未来里程碑 |

### 会话

| 命令 | 作用 |
|------|------|
| `/gsdt:pause-work` | 在中途暂停时创建交接上下文（写入 HANDOFF.json） |
| `/gsdt:resume-work` | 从上一次会话恢复 |
| `/gsdt:session-report` | 生成会话摘要，包含已完成工作和结果 |

### 工具

| 命令 | 作用 |
|------|------|
| `/gsdt:settings` | 配置模型 profile 和工作流代理 |
| `/gsdt:set-profile <profile>` | 切换模型 profile（quality / balanced / budget / inherit） |
| `/gsdt:add-todo [desc]` | 记录一个待办想法 |
| `/gsdt:check-todos` | 查看待办列表 |
| `/gsdt:debug [desc]` | 使用持久状态进行系统化调试 |
...
GSDT 将项目设置保存在 `.claude/.gsdt-planning/config.json`。你可以在 `/gsdt:new-project` 时配置，也可以稍后通过 `/gsdt:settings` 修改。完整的配置 schema、工作流开关、git branching 选项以及各代理的模型分配，请查看[用户指南](docs/USER-GUIDE.md#configuration-reference)。
...
可以用 `/gsdt:settings` 开关这些项，也可以在单次命令里覆盖：
...
在里程碑完成时，GSDT 会提供 squash merge（推荐）或保留历史的 merge 选项。
...
GSDT 的代码库映射和分析命令会读取文件来理解你的项目。**包含机密信息的文件应当加入 Claude Code 的 deny list**：
...
> [!IMPORTANT]
> GSDT 内建了防止提交 secrets 的保护，但纵深防御依然是最佳实践。第一道防线应该是直接禁止读取敏感文件。
...
- 检查文件是否存在于 `~/.claude/commands/gsdt/`（全局）或 `./.claude/commands/gsdt/`（本地）
- 对 Codex，检查 skills 是否存在于 `~/.codex/skills/gsdt-*/SKILL.md`（全局）或 `./.codex/skills/gsdt-*/SKILL.md`（本地）
...
这会移除所有 GSDT 命令、代理、hooks 和设置，但会保留你其他配置。
