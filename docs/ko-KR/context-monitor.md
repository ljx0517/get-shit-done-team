...
| CRITICAL | <= 25% | 즉시 중단 후 상태 저장 (`/gsdt:pause-work`) |
...
```
Statusline Hook (gsdt-statusline.js)
    | 기록
    v
/tmp/claude-ctx-{session_id}.json
    ^ 읽기
    |
Context Monitor (gsdt-context-monitor.js, PostToolUse/AfterTool)
    | 주입
    v
...
GSD의 `/gsdt:pause-work` 명령어는 실행 상태를 저장합니다. WARNING 메시지는 해당 명령어 사용을 권장하며 CRITICAL 메시지는 즉각적인 상태 저장을 지시합니다.

## 설정

두 훅 모두 `npx gsdt` 설치 중에 자동으로 등록됩니다.
...
```json
{
  "statusLine": {
    "type": "command",
    "command": "node ~/.claude/hooks/gsdt-statusline.js"
  },
  "hooks": {
    "PostToolUse": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "node ~/.claude/hooks/gsdt-context-monitor.js"
          }
        ]
      }
    ]
    ...
    "AfterTool": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "node ~/.gemini/hooks/gsdt-context-monitor.js"
          }
        ]
      }
    ]
