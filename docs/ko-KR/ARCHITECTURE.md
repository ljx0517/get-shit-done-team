...
워크플로우 파일(`gsdt/workflows/*.md`)은 무거운 작업을 직접 수행하지 않습니다. 다음 작업만 담당합니다.
- `gsdt-tools.cjs init <workflow>`로 컨텍스트를 로드합니다
...
- `gsdt-tools.cjs init`을 통한 컨텍스트 로드
...
모든 계획 아티팩트를 위한 Markdown 템플릿입니다. `gsdt-tools.cjs template fill`과 `scaffold` 명령어가 사전 구조화된 파일을 생성하는 데 사용합니다.
...
| `gsdt-prompt-guard.js` | `PreToolUse` | `.planning/` 쓰기 작업에서 프롬프트 인젝션 패턴 스캔 (권고용) |
| `gsdt-workflow-guard.js` | `PreToolUse` | GSD 워크플로우 컨텍스트 외부의 파일 편집 감지 (권고용, `hooks.workflow_guard`로 활성화) |

### CLI Tools (`gsdt/bin/`)

17개의 도메인 모듈을 포함하는 Node.js CLI 유틸리티(`gsdt-tools.cjs`)입니다.
...
| 카테고리 | 에이전트 | 병렬성 |
|----------|--------|-------------|
| **Researchers** | gsdt-project-researcher, gsdt-phase-researcher, gsdt-ui-researcher, gsdt-advisor-researcher | 4개 병렬 (stack, features, architecture, pitfalls); advisor는 discuss-phase 중 생성됨 |
| **Synthesizers** | gsdt-research-synthesizer | 순차적 (조사자 완료 후) |
| **Planners** | gsdt-planner, gsdt-roadmapper | 순차적 |
| **Checkers** | gsdt-plan-checker, gsdt-integration-checker, gsdt-ui-checker, gsdt-nyquist-auditor | 순차적 (검증 루프, 최대 3회 반복) |
| **Executors** | gsdt-executor | 웨이브 내 병렬, 웨이브 간 순차적 |
| **Verifiers** | gsdt-verifier | 순차적 (모든 executor 완료 후) |
| **Mappers** | gsdt-codebase-mapper | 4개 병렬 (tech, arch, quality, concerns) |
| **Debuggers** | gsdt-debugger | 순차적 (대화형) |
| **Auditors** | gsdt-ui-auditor | 순차적 |
...
├── commands/gsdt/*.md               # 37개 슬래시 명령어
├── gsdt/
│   ├── bin/gsdt-tools.cjs           # CLI 유틸리티
...
├── hooks/
│   ├── gsdt-statusline.js           # 상태표시줄 훅
│   ├── gsdt-context-monitor.js      # 컨텍스트 경고 훅
│   └── gsdt-check-update.js         # 업데이트 확인 훅
├── settings.json                   # 훅 등록
└── VERSION                         # 설치된 버전 번호
```
...
├── research/               # /gsdt:new-project의 도메인 조사
│   ├── SUMMARY.md
│   ├── STACK.md
│   ├── FEATURES.md
│   ├── ARCHITECTURE.md
│   └── PITFALLS.md
├── codebase/               # 브라운필드 매핑 (/gsdt:map-codebase에서)
│   ├── STACK.md
│   ├── ARCHITECTURE.md
│   ├── CONVENTIONS.md
│   ├── CONCERNS.md
...
│   ├── pending/            # 캡처된 아이디어
│   └── done/               # 완료된 할 일
├── threads/               # 영구 컨텍스트 스레드 (/gsdt:thread에서)
├── seeds/                 # 미래 지향적 아이디어 (/gsdt:plant-seed에서)
├── debug/                  # 활성 디버그 세션
│   ├── *.md                # 활성 세션
│   ├── resolved/           # 보관된 세션
│   └── knowledge-base.md   # 영구 디버그 학습 내용
├── ui-reviews/             # /gsdt:ui-review의 스크린샷 (gitignored)
