...
**출력:** `.claude/.gsdt-planning/phases/{phase-dir}/`의 `{padded_phase}-UI-SPEC.md`
...
`/gsdt:ui-review`는 Playwright CLI를 통해 `.claude/.gsdt-planning/ui-reviews/`에 스크린샷을 캡처합니다. 바이너리 파일이 git에 포함되지 않도록 `.gitignore`가 자동으로 생성됩니다. 스크린샷은 `/gsdt:complete-milestone` 실행 시 정리됩니다.
...
**저장 위치:** `.claude/.gsdt-planning/seeds/SEED-NNN-slug.md`
...
**저장 위치:** `.claude/.gsdt-planning/threads/{slug}.md`

---

## 워크스트림

워크스트림을 사용하면 상태 충돌 없이 여러 마일스톤 영역을 동시에 작업할 수 있습니다. 각 워크스트림은 독립적인 `.claude/.gsdt-planning/` 상태를 가지므로 워크스트림 간 전환 시 진행 상황이 덮어쓰이지 않습니다.
...
각 워크스트림은 자체 `.claude/.gsdt-planning/` 디렉터리 하위 트리를 유지합니다. 워크스트림을 전환하면 GSD가 활성 계획 컨텍스트를 교체하여 `/gsdt:progress`, `/gsdt:discuss-phase`, `/gsdt:plan-phase` 및 기타 명령어가 해당 워크스트림의 상태로 동작합니다.
...
**런타임 훅.**
- `gsdt-prompt-guard.js` — `.claude/.gsdt-planning/`에 대한 Write/Edit 호출에서 인젝션 패턴 스캔 (항상 활성, 권고만)
...
| `/gsdt:pr-branch` | `.claude/.gsdt-planning/` 커밋을 필터링한 깔끔한 PR 브랜치 | 계획 없는 diff로 PR 생성 전 |
...
GSD는 프로젝트 설정을 `.claude/.gsdt-planning/config.json`에 저장합니다. `/gsdt:new-project` 중에 설정하거나 나중에 `/gsdt:settings`로 업데이트할 수 있습니다.
...
| 설정 | 옵션 | 기본값 | 제어 대상 |
|------|------|--------|----------|
| `planning.commit_docs` | `true`, `false` | `true` | `.claude/.gsdt-planning/` 파일을 git에 커밋할지 여부 |
| `planning.search_gitignored` | `true`, `false` | `false` | 광범위한 검색에 `--no-ignore`를 추가하여 `.claude/.gsdt-planning/` 포함 |

> **참고:** `.claude/.gsdt-planning/`이 `.gitignore`에 있으면 설정 값에 관계없이 `commit_docs`는 자동으로 `false`가 됩니다.
...
각 워크스페이스는 다음을 포함합니다.
- 자체 `.claude/.gsdt-planning/` 디렉터리 (원본 저장소와 완전히 독립)
...
`.claude/.gsdt-planning/PROJECT.md`가 이미 존재하는데 `/gsdt:new-project`를 실행했습니다. 이것은 안전 검사입니다. 처음부터 다시 시작하려면 먼저 `.claude/.gsdt-planning/` 디렉터리를 삭제하세요.
...
비Claude 런타임에서 에이전트별로 다른 모델을 할당하려면 런타임이 인식하는 완전한 자격을 갖춘 모델 ID와 함께 `.claude/.gsdt-planning/config.json`에 `model_overrides`를 추가하세요.
...
설치 프로그램은 Gemini CLI, OpenCode, Codex에 대해 `resolve_model_ids: "omit"`을 자동으로 구성합니다. 비Claude 런타임을 수동으로 설정하는 경우 직접 `.claude/.gsdt-planning/config.json`에 추가하세요.
...
`/gsdt:new-project` 중에 또는 `/gsdt:settings`에서 `commit_docs: false`로 설정하세요. `.claude/.gsdt-planning/`을 `.gitignore`에 추가하세요. 계획 아티팩트는 로컬에 유지되며 git에 절대 포함되지 않습니다.
...
**출력:** 발견사항과 권장 수정 단계가 포함된 `.claude/.gsdt-planning/forensics/`의 진단 보고서.
