...
**명령어:** `/gsdt:new-project [--auto @file.md]`
...
- 기존 코드베이스 컨텍스트(`/gsdt:map-codebase`에서)가 있으면 로드됩니다.

---

### 2. Phase Discussion

**명령어:** `/gsdt:discuss-phase [N] [--auto] [--batch]`
...
**명령어:** `/gsdt:plan-phase [N] [--auto] [--skip-research] [--skip-verify]`
...
- REQ-PLAN-07: 프론트엔드 페이즈가 감지되고 UI-SPEC.md가 없는 경우 `/gsdt:ui-phase` 실행을 촉구해야 합니다(UI 안전 게이트).
...
**명령어:** `/gsdt:audit-milestone`, `/gsdt:complete-milestone`, `/gsdt:new-milestone [name]`
...
**갭 해소.** `/gsdt:plan-milestone-gaps`는 감사에서 식별된 갭을 해소하는 페이즈를 생성합니다.
...
**명령어:** `/gsdt:add-phase`, `/gsdt:insert-phase [N]`, `/gsdt:remove-phase [N]`
...
**명령어:** `/gsdt:quick [--full] [--discuss] [--research]`

**목적:** GSDT 보증을 제공하지만 더 빠른 경로로 임시 작업을 실행합니다.
...
**목적:** 자유형 텍스트를 분석하고 적절한 GSDT 명령어로 라우팅합니다.

**요구사항.**
- REQ-DO-01: 자연어 입력에서 사용자 의도를 파악해야 합니다.
- REQ-DO-02: 의도를 가장 적합한 GSDT 명령어에 매핑해야 합니다.
...
- REQ-NEXT-04: 프로젝트가 없으면 `/gsdt:new-project`를 제안해야 합니다.
- REQ-NEXT-05: 모든 페이즈가 완료되면 `/gsdt:complete-milestone`을 제안해야 합니다.
...
| 실행되었지만 VERIFICATION.md 없음 | `/gsdt:verify-work` 실행 |
...
- REQ-POSTVER-03: `/gsdt:verify-work`가 처리할 문제를 기록해야 합니다.
...
**실행 시점.** `/gsdt:execute-phase` 중 검증자 단계 전에 자동으로 실행됩니다.
...
**실행 시점.** `/gsdt:plan-phase`의 계획 검사기 루프 후 자동으로 실행됩니다.
...
- REQ-CTX-06: 컨텍스트 모니터는 GSDT 활성 프로젝트와 비활성 프로젝트를 구분해야 합니다.
...
**명령어:** `/gsdt:pause-work`, `/gsdt:resume-work`, `/gsdt:progress`
...
**명령어:** `/gsdt:set-profile <quality|balanced|budget|inherit>`
...
**명령어:** `/gsdt:add-todo [desc]`, `/gsdt:check-todos`
...
- REQ-UPDATE-04: 로컬에서 수정된 파일을 `gsdt-local-patches/`에 백업해야 합니다.
- REQ-UPDATE-05: `/gsdt:reapply-patches`는 업데이트 후 로컬 수정사항을 복원해야 합니다.
...
- REQ-RUNTIME-05: 제거는 다른 구성에 영향을 주지 않고 모든 GSDT 파일을 깔끔하게 제거해야 합니다.
...
**명령어:** `/gsdt:profile-user [--questionnaire] [--refresh]`
...
- `/gsdt:dev-preferences` 명령어 — 모든 세션에서 선호도 로드
...
- `gsdt-user-profiler` 에이전트 — 세션 데이터에서 행동 분석
...
모든 `/gsdt:progress` 호출은 현재 마일스톤의 모든 페이즈에서 미결 항목(pending, skipped, blocked, human_needed)을 스캔합니다. 실행 가능한 링크가 포함된 비차단 경고 섹션을 표시합니다.
...
**요구사항.**
- REQ-DEBT-01: `/gsdt:progress`에서 모든 이전 페이즈의 미결 UAT/검증 항목을 표시해야 합니다.
...
- REQ-DEBT-06: `/gsdt:audit-uat`는 모든 페이즈를 스캔하고 테스트 가능성별로 항목을 분류하며 사람 테스트 계획을 생성해야 합니다.
...
**명령어:** `/gsdt:fast [task description]`
...
- `/gsdt:fast` — 2분 이내에 실행 가능한 한 문장 작업(오타, 구성 변경, 작은 추가)
- `/gsdt:quick` — 연구, 다단계 계획, 또는 검증이 필요한 모든 것

---

### 42. Cross-AI Peer Review

**명령어:** `/gsdt:review --phase N [--gemini] [--claude] [--codex] [--all]`
...
- REQ-REVIEW-05: 검토는 `/gsdt:plan-phase --reviews`가 사용할 수 있어야 합니다.
...
**명령어:** `/gsdt:add-backlog <description>`, `/gsdt:review-backlog`, `/gsdt:plant-seed <idea>`
...
- REQ-BACKLOG-02: `/gsdt:discuss-phase`와 `/gsdt:plan-phase`가 작동할 수 있도록 페이즈 디렉토리를 즉시 생성해야 합니다.
- REQ-BACKLOG-03: `/gsdt:review-backlog`는 항목별로 승격, 유지, 제거 액션을 지원해야 합니다.
- REQ-BACKLOG-04: 승격된 항목은 활성 마일스톤 순서로 번호가 다시 매겨져야 합니다.
- REQ-SEED-01: 시드는 표시 조건에 대한 전체 이유와 시기를 캡처해야 합니다.
- REQ-SEED-02: `/gsdt:new-milestone`은 시드를 스캔하고 일치하는 항목을 표시해야 합니다.
...
**명령어:** `/gsdt:thread [name | description]`

**목적:** 여러 세션에 걸쳐 있지만 특정 페이즈에 속하지 않는 작업을 위한 가벼운 교차 세션 지식 저장소입니다. `/gsdt:pause-work`보다 더 가볍습니다 — 페이즈 상태나 계획 컨텍스트가 없습니다.
...
**목적:** GSDT 계획 산출물에 대한 심층 방어 보안. GSD가 LLM 시스템 프롬프트가 되는 마크다운 파일을 생성하기 때문에, 이 파일로 흘러드는 사용자 제어 텍스트는 잠재적인 간접 프롬프트 주입 벡터입니다.
...
**목적:** `/gsdt:discuss-phase` 중에 `DISCUSSION-LOG.md`를 자동 생성하여 논의 중에 내려진 결정의 전체 감사 추적을 제공합니다.
...
**명령어:** `/gsdt:forensics [description]`

**목적:** 실패하거나 막힌 GSDT 워크플로우의 사후 조사.
...
**명령어:** `/gsdt:milestone-summary [version]`
...
**명령어:** `/gsdt:discuss-phase` with `workflow.discuss_mode: 'assumptions'`
...
**일부:** `/gsdt:new-project` 및 `/gsdt:progress`

**목적:** UI 중심 프로젝트를 자동으로 감지하고 `/gsdt:ui-phase` 권장사항을 표시합니다.

**요구사항.**
- REQ-UI-DETECT-01: 프로젝트 설명에서 UI 신호를 감지해야 합니다(키워드, 프레임워크 참조).
- REQ-UI-DETECT-02: 해당하는 경우 ROADMAP.md 페이즈에 `ui_hint`를 주석으로 추가해야 합니다.
- REQ-UI-DETECT-03: UI 중심 페이즈의 다음 단계에서 `/gsdt:ui-phase`를 제안해야 합니다.
- REQ-UI-DETECT-04: `/gsdt:ui-phase`를 필수로 만들어서는 안 됩니다.

**프로세스.**
1. **감지** — UI 신호(키워드, 프레임워크 참조)에 대한 프로젝트 설명 및 기술 스택 스캔
2. **주석** — ROADMAP.md의 해당 페이즈에 `ui_hint` 표시 추가
3. **표시** — UI 중심 페이즈의 다음 단계에 `/gsdt:ui-phase` 권장사항 포함
...
3. **설치** — 단일 세션에서 선택된 모든 런타임에 GSDT 구성
