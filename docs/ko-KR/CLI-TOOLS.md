...
`gsdt-tools.cjs`는 GSD의 약 50개 명령어, 워크플로우, 에이전트 파일에서 반복되는 인라인 bash 패턴을 대체하는 Node.js CLI 유틸리티입니다. config 파싱, 모델 해석, 단계 조회, git 커밋, 요약 검증, 상태 관리, 템플릿 작업을 중앙화합니다.

**위치:** `gsdt/bin/gsdt-tools.cjs`
**모듈:** `gsdt/bin/lib/`의 15개 도메인 모듈
...
`.gsdt-planning/STATE.md`를 관리합니다 — 프로젝트의 살아있는 메모리입니다.
...
`.gsdt-planning/config.json`을 읽고 씁니다.
...
에이전트 이름: `gsdt-planner`, `gsdt-executor`, `gsdt-phase-researcher`, `gsdt-project-researcher`, `gsdt-research-synthesizer`, `gsdt-verifier`, `gsdt-plan-checker`, `gsdt-integration-checker`, `gsdt-roadmapper`, `gsdt-debugger`, `gsdt-codebase-mapper`, `gsdt-nyquist-auditor`
...
**대용량 페이로드 처리:** 출력이 약 50KB를 초과하면 CLI가 임시 파일에 쓰고 `@file:/tmp/gsdt-init-XXXXX.json`을 반환합니다. 워크플로우는 `@file:` 접두사를 확인하고 디스크에서 읽습니다.
