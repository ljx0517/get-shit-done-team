...
**사전 조건:** `.gsdt-planning/PROJECT.md`가 존재하지 않아야 합니다.
...
격리된 워크스페이스를 생성합니다. 저장소 복사본과 독립적인 `.gsdt-planning/` 디렉터리가 포함됩니다.
...
**사용 사례.**
- 멀티 저장소: 격리된 GSDT 상태로 일부 저장소만 작업합니다.
- 기능 격리: `--repos .`는 현재 저장소의 worktree를 생성합니다.

**생성 파일:** `WORKSPACE.md`, `.gsdt-planning/`, 저장소 복사본 (worktree 또는 clone)
...
활성 GSDT 워크스페이스와 상태를 목록으로 표시합니다.

**스캔 위치:** `~/gsdt-workspaces/`에서 `WORKSPACE.md` 매니페스트를 탐색합니다.
**표시 항목:** 이름, 저장소 수, 전략, GSDT 프로젝트 상태
...
**사전 조건:** `.gsdt-planning/ROADMAP.md`가 존재해야 합니다.
...
**사전 조건:** `.gsdt-planning/ROADMAP.md`가 존재해야 하며 해당 페이즈에 프론트엔드/UI 작업이 포함되어야 합니다.
...
**사전 조건:** `.gsdt-planning/` 디렉터리가 존재해야 합니다.
**동작 방식.**
...
**생성 파일:** `.gsdt-planning/reports/SESSION_REPORT.md`
...
**사전 조건:** 프론트엔드 코드가 있는 프로젝트 (독립 실행 가능, GSDT 프로젝트 불필요)
**생성 파일:** `{phase}-UI-REVIEW.md`, `.gsdt-planning/ui-reviews/`에 스크린샷
...
**생성 파일:** `.gsdt-planning/reports/MILESTONE_SUMMARY-v{version}.md`
...
자유 형식 텍스트를 적절한 GSDT 명령어로 라우팅합니다.
...
`.gsdt-planning/` 디렉터리의 무결성을 검사합니다.
...
실패하거나 멈춘 GSDT 워크플로우에 대한 사후 조사를 수행합니다.
...
**사전 조건:** `.gsdt-planning/` 디렉터리가 존재해야 합니다.
**생성 파일:** `.gsdt-planning/forensics/report-{timestamp}.md`
...
**사전 조건:** 활성 GSDT 프로젝트
**생성 파일:** `.gsdt-planning/` 하위의 워크스트림 디렉터리, 워크스트림별 상태 추적
...
핵심 전략 기본값, 워크플로우 토글, 토론 모드, 모델 프로필을 대화형으로 설정합니다.
...
`.gsdt-planning/` 커밋을 필터링한 깔끔한 PR 브랜치를 생성합니다.

| 인수 | 필수 여부 | 설명 |
|------|----------|------|
| `target branch` | 아니오 | 기본 브랜치 (기본값: `main`) |

**목적:** 리뷰어에게 GSDT 계획 아티팩트가 아닌 코드 변경사항만 표시합니다.
...
**생성 파일:** `.gsdt-planning/seeds/SEED-NNN-slug.md`
