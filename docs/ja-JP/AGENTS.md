...
GSDT はマルチエージェントアーキテクチャを採用しており、軽量なオーケストレーター（ワークフローファイル）が新しいコンテキストウィンドウを持つ専門エージェントをスポーンします。各エージェントは特定の役割に特化し、限定的なツールアクセス権を持ち、特定の成果物を生成します。
...
### gsdt-project-researcher

**役割:** ロードマップ作成前にドメインエコシステムを調査する。

| プロパティ | 値 |
|------------|-----|
| **スポーン元** | `/gsdt:new-project`, `/gsdt:new-milestone` |
| **並列数** | 4インスタンス（stack, features, architecture, pitfalls） |
| **ツール** | Read, Write, Bash, Grep, Glob, WebSearch, WebFetch, mcp (context7) |
| **モデル (balanced)** | Sonnet |
| **生成物** | `.claude/.gsdt-planning/research/STACK.md`, `FEATURES.md`, `ARCHITECTURE.md`, `PITFALLS.md` |
...
### gsdt-phase-researcher

**役割:** 計画策定前に、特定フェーズの実装方法を調査する。

| プロパティ | 値 |
|------------|-----|
| **スポーン元** | `/gsdt:plan-phase` |
...
### gsdt-research-synthesizer

**役割:** 並列リサーチャーの出力を統合サマリーにまとめる。

| プロパティ | 値 |
|------------|-----|
| **スポーン元** | `/gsdt:new-project`（4つのリサーチャー完了後） |
| **並列数** | 単一インスタンス（リサーチャー後に順次実行） |
| **ツール** | Read, Write, Bash |
| **モデル (balanced)** | Sonnet |
| **カラー** | Purple |
| **生成物** | `.claude/.gsdt-planning/research/SUMMARY.md` |
...
| プロパティ | 値 |
|------------|-----|
| **スポーン元** | `/gsdt:plan-phase`, `/gsdt:quick` |
| **並列数** | 単一インスタンス |
...
**役割:** アトミックコミット、逸脱処理、チェックポイントプロトコルを使用して GSDT 計画を実行する。

| プロパティ | 値 |
|------------|-----|
| **スポーン元** | `/gsdt:execute-phase`, `/gsdt:quick` |
...
| プロパティ | 値 |
|------------|-----|
| **スポーン元** | `/gsdt:plan-phase`（検証ループ、最大3回の反復） |
| **並列数** | 単一インスタンス（反復型） |
...
### gsdt-integration-checker

**役割:** フェーズ間の統合とエンドツーエンドフローを検証する。

| プロパティ | 値 |
|------------|-----|
| **スポーン元** | `/gsdt:audit-milestone` |
| **並列数** | 単一インスタンス |
| **ツール** | Read, Bash, Grep, Glob |
| **モデル (balanced)** | Sonnet |
| **カラー** | Blue |
...
| プロパティ | 値 |
|------------|-----|
| **スポーン元** | `/gsdt:ui-phase`（検証ループ、最大2回の反復） |
| **並列数** | 単一インスタンス |
| **ツール** | Read, Bash, Glob, Grep |
| **モデル (balanced)** | Sonnet |
| **カラー** | `#22D3EE`（シアン） |
...
| プロパティ | 値 |
|------------|-----|
| **スポーン元** | `/gsdt:execute-phase`（すべてのエグゼキューター完了後） |
| **並列数** | 単一インスタンス |
...
- `/gsdt:verify-work` で対処すべき問題をログに記録

---

### gsdt-nyquist-auditor

**役割:** テストを生成して Nyquist バリデーションのギャップを埋める。

| プロパティ | 値 |
|------------|-----|
| **スポーン元** | `/gsdt:validate-phase` |
| **並列数** | 単一インスタンス |
| **ツール** | Read, Write, Edit, Bash, Grep, Glob |
| **モデル (balanced)** | Sonnet |
...
| プロパティ | 値 |
|------------|-----|
| **スポーン元** | `/gsdt:debug`, `/gsdt:verify-work`（失敗時） |
| **並列数** | 単一インスタンス（インタラクティブ） |
| **ツール** | Read, Write, Edit, Bash, Grep, Glob, WebSearch |
| **モデル (balanced)** | Sonnet |
| **カラー** | Orange |
| **生成物** | `.claude/.gsdt-planning/debug/*.md`、ナレッジベースの更新 |
...
| プロパティ | 値 |
|------------|-----|
| **スポーン元** | `/gsdt:profile-user` |
| **並列数** | 単一インスタンス |
| **ツール** | Read |
| **モデル (balanced)** | Sonnet |
| **カラー** | Magenta |
| **生成物** | `USER-PROFILE.md`、`/gsdt:dev-preferences`、`CLAUDE.md` プロファイルセクション |
