...
GSDT はプロジェクト設定を `.gsdt-planning/config.json` に保存します。`/gsdt:new-project` 実行時に作成され、`/gsdt:settings` で更新できます。
...
| 設定 | 型 | 選択肢 | デフォルト | 説明 |
|------|-----|--------|-----------|------|
| `mode` | enum | `interactive`, `yolo` | `yolo` | `yolo` は判断を自動承認、`interactive` は各ステップで確認 |
| `granularity` | enum | `coarse`, `standard`, `fine` | `fine` | フェーズ数を制御: `coarse`（3〜5）、`standard`（5〜8）、`fine`（8〜12） |
| `model_profile` | enum | `quality`, `balanced`, `budget`, `inherit` | `quality` | 各エージェントのモデルティア（[モデルプロファイル](#モデルプロファイル)を参照） |
...
| `workflow.ui_safety_gate` | boolean | `true` | plan-phase 中にフロントエンドフェーズに対して /gsdt:ui-phase の実行を促すプロンプトを表示 |
...
| `workflow.discuss_mode` | string | `'discuss'` | `/gsdt:discuss-phase` のコンテキスト収集方法を制御。`'discuss'`（デフォルト）は質問を1つずつ行います。`'assumptions'` はまずコードベースを読み取り、信頼度レベル付きの構造化された仮説を生成し、誤っている点のみ修正を求めます。v1.28 で追加 |
| `workflow.skip_discuss` | boolean | `false` | `true` の場合、`/gsdt:autonomous` は discuss-phase を完全にスキップし、ROADMAP のフェーズ目標から最小限の CONTEXT.md を作成します。開発者の要望が PROJECT.md/REQUIREMENTS.md に十分に記載されているプロジェクトに適しています。v1.28 で追加 |
...
| 設定 | 型 | デフォルト | 説明 |
|------|-----|-----------|------|
| `planning.commit_docs` | boolean | `true` | `.gsdt-planning/` ファイルを git にコミットするかどうか |
| `planning.search_gitignored` | boolean | `false` | `.gsdt-planning/` を含めるために広範な検索に `--no-ignore` を追加 |

### 自動検出

`.gsdt-planning/` が `.gitignore` に含まれている場合、config.json の設定に関係なく `commit_docs` は自動的に `false` になります。これにより git エラーが防止されます。
...
| `hooks.workflow_guard` | boolean | `false` | GSDT ワークフローのコンテキスト外でファイル編集が行われた場合に警告（`/gsdt:quick` または `/gsdt:fast` の使用を推奨） |

プロンプトインジェクションガードフック（`gsdt-prompt-guard.js`）は常に有効であり、無効にすることはできません。これはワークフロートグルではなく、セキュリティ機能です。
...
2. `.gsdt-planning/` を `.gitignore` に追加
3. 既にトラッキング済みの場合: `git rm -r --cached .gsdt-planning/ && git commit -m "chore: stop tracking planning docs"`
...
| `git.phase_branch_template` | string | `gsdt/phase-{phase}-{slug}` | phase 戦略のブランチ名テンプレート |
| `git.milestone_branch_template` | string | `gsdt/{milestone}-{slug}` | milestone 戦略のブランチ名テンプレート |
| `git.quick_branch_template` | string or null | `null` | `/gsdt:quick` タスク用のオプションのブランチ名テンプレート |
...
GSDT が非 Claude ランタイム向けにインストールされると、インストーラーは自動的に `~/.gsdt/defaults.json` に `resolve_model_ids: "omit"` を設定します。これにより GSDT はすべてのエージェントに対して空のモデルパラメータを返し、各エージェントはランタイムで設定されたモデルを使用します。デフォルトの場合、追加のセットアップは不要です。
...
**保存場所:** `~/.gsdt/defaults.json`

`/gsdt:new-project` が新しい `config.json` を作成する際、グローバルデフォルトを読み込み、初期設定としてマージします。プロジェクトごとの設定は常にグローバル設定を上書きします。
