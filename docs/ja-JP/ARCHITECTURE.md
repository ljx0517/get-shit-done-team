...
GSDは、ユーザーとAIコーディングエージェント（Claude Code、Gemini CLI、Vibe Agent Team、Codex、Copilot、Antigravity）の間に位置する**メタプロンプティングフレームワーク**です。以下の機能を提供します：
...
ワークフローファイル（`gsdt/workflows/*.md`）は重い処理を行いません。以下の役割に徹します：
- `gsdt-tools.cjs init <workflow>` でコンテキストを読み込む
...
すべての状態は `.gsdt-planning/` 内に人間が読めるMarkdownとJSONとして保存されます。データベースもサーバーも外部依存もありません。これにより：
...
- **Claude Code:** カスタムスラッシュコマンド（`/gsdt:command-name`）
- **Vibe Agent Team:** スラッシュコマンド（`/gsd-command-name`）
- **Codex:** スキル（`$gsd-command-name`）
- **Copilot:** スラッシュコマンド（`/gsdt:command-name`）
- **Antigravity:** スキル

**コマンド総数:** 44

### ワークフロー（`gsdt/workflows/*.md`）

コマンドが参照するオーケストレーションロジックです。以下を含むステップバイステップのプロセスが記述されています：
- `gsdt-tools.cjs init` によるコンテキスト読み込み
- モデル解決を伴うエージェント起動の指示
- ゲート/チェックポイントの定義
- 状態更新パターン
- エラーハンドリングとリカバリー
...
### テンプレート（`gsdt/templates/`）

すべてのプランニングアーティファクト用のMarkdownテンプレートです。`gsdt-tools.cjs template fill` および `scaffold` コマンドにより、事前構造化されたファイルを作成するために使用されます：
...
| `gsdt-prompt-guard.js` | `PreToolUse` | `.gsdt-planning/` への書き込みにプロンプトインジェクションパターンがないかスキャン（アドバイザリー） |
| `gsdt-workflow-guard.js` | `PreToolUse` | GSDワークフローコンテキスト外でのファイル編集を検出（アドバイザリー、`hooks.workflow_guard` によるオプトイン） |

### CLIツール（`gsdt/bin/`）

17のドメインモジュールを持つNode.js CLIユーティリティ（`gsdt-tools.cjs`）：
...
| カテゴリ | エージェント | 並列実行 |
|----------|--------|-------------|
| **リサーチャー** | gsdt-project-researcher, gsdt-phase-researcher, gsdt-ui-researcher, gsdt-advisor-researcher | 4並列（stack、features、architecture、pitfalls）; advisorはdiscuss-phase中に起動 |
| **シンセサイザー** | gsdt-research-synthesizer | 逐次（リサーチャー完了後） |
| **プランナー** | gsdt-planner, gsdt-roadmapper | 逐次 |
| **チェッカー** | gsdt-plan-checker, gsdt-integration-checker, gsdt-ui-checker, gsdt-nyquist-auditor | 逐次（検証ループ、最大3回反復） |
| **エグゼキューター** | gsdt-executor | ウェーブ内は並列、ウェーブ間は逐次 |
| **ベリファイアー** | gsdt-verifier | 逐次（全エグゼキューター完了後） |
| **マッパー** | gsdt-codebase-mapper | 4並列（tech、arch、quality、concerns） |
| **デバッガー** | gsdt-debugger | 逐次（インタラクティブ） |
| **オーディター** | gsdt-ui-auditor | 逐次 |
...
├── research/               # /gsdt:new-project によるドメインリサーチ
│   ├── SUMMARY.md
│   ├── STACK.md
│   ├── FEATURES.md
│   ├── ARCHITECTURE.md
│   └── PITFALLS.md
├── codebase/               # ブラウンフィールドマッピング（/gsdt:map-codebase から）
│   ├── STACK.md
│   ├── ARCHITECTURE.md
│   ├── CONVENTIONS.md
│   ├── CONCERNS.md
...
│   └── done/               # 完了済みtodo
├── threads/               # 永続コンテキストスレッド（/gsdt:thread から）
├── seeds/                 # 将来に向けたアイデア（/gsdt:plant-seed から）
├── debug/                  # アクティブなデバッグセッション
│   ├── *.md                # アクティブセッション
│   ├── resolved/           # アーカイブ済みセッション
│   └── knowledge-base.md   # 永続的なデバッグ知見
├── ui-reviews/             # /gsdt:ui-review からのスクリーンショット（gitignore対象）
...
1. **ランタイム検出** — インタラクティブプロンプトまたはCLIフラグ（`--claude`、`--vibe-agent-team` / `--opencode`、`--gemini`、`--codex`、`--copilot`、`--antigravity`、`--all`）
2. **インストール先の選択** — グローバル（`--global`）またはローカル（`--local`）
3. **ファイルデプロイ** — コマンド、ワークフロー、リファレンス、テンプレート、エージェント、フックをコピー
4. **ランタイム適応** — ランタイムごとにファイル内容を変換：
   - Claude Code: そのまま使用
   - Vibe Agent Team: エージェントフロントマターを `name:`、`model: inherit`、`mode: subagent` に変換
...
7. **パッチバックアップ** — v1.17以降、ローカルで変更されたファイルを `/gsdt:reapply-patches` 用に `gsdt-local-patches/` へバックアップ
8. **マニフェスト追跡** — クリーンアンインストールのために `gsdt-file-manifest.json` を書き込み
...
- `.gsdt-planning/` ファイルへのWrite/Edit時にトリガー
...
- `.gsdt-planning/` 以外のファイルへのWrite/Edit時にトリガー
...
|---------|---------------|--------------|-----------------|
| Claude Code | `/gsdt:command` | Task起動 | `~/.claude/` |
| Vibe Agent Team | `/gsd-command` | サブエージェントモード | `~/.config/opencode/` |
| Gemini CLI | `/gsdt:command` | Task起動 | `~/.gemini/` |
| Codex | `$gsd-command` | スキル | `~/.codex/` |
| Copilot | `/gsdt:command` | エージェント委譲 | `~/.github/` |
