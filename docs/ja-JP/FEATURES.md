...
**コマンド:** `/gsdt:new-project [--auto @file.md]`
...
- REQ-INIT-06: `.claude/.gsdt-planning/PROJECT.md` が既に存在する場合、システムは再初期化を防止しなければならない
...
- 既存のコードベースコンテキスト（`/gsdt:map-codebase` から取得）がある場合は読み込む

---

### 2. フェーズディスカッション

**コマンド:** `/gsdt:discuss-phase [N] [--auto] [--batch]`
...
**コマンド:** `/gsdt:plan-phase [N] [--auto] [--skip-research] [--skip-verify]`
...
- REQ-PLAN-07: フロントエンドフェーズが検出され UI-SPEC.md が存在しない場合、システムはユーザーに `/gsdt:ui-phase` の実行を促さなければならない（UI セーフティゲート）
...
**コマンド:** `/gsdt:ui-review [N]`

**目的:** 実装済みフロントエンドコードに対する遡及的な6本柱のビジュアル監査。任意のプロジェクトでスタンドアロンで動作します。

**要件:**
- REQ-UIREVIEW-01: システムは6つの柱それぞれを1〜4のスケールで評価しなければならない
- REQ-UIREVIEW-02: システムは Playwright CLI を使用して `.claude/.gsdt-planning/ui-reviews/` にスクリーンショットをキャプチャしなければならない
...
**コマンド:** `/gsdt:audit-milestone`、`/gsdt:complete-milestone`、`/gsdt:new-milestone [name]`
...
**ギャップクローズ:** `/gsdt:plan-milestone-gaps` は監査で特定されたギャップを埋めるためのフェーズを作成します。
...
**コマンド:** `/gsdt:add-phase`、`/gsdt:insert-phase [N]`、`/gsdt:remove-phase [N]`
...
**コマンド:** `/gsdt:quick [--full] [--discuss] [--research]`

**目的:** GSDT の保証を維持しながら、より高速なパスでアドホックなタスクを実行します。
...
- REQ-QUICK-08: システムは Quick タスクを `.claude/.gsdt-planning/quick/YYMMDD-xxx-slug/` で追跡しなければならない
...
**目的:** 自由形式のテキストを分析し、適切な GSDT コマンドにルーティングします。

**要件:**
- REQ-DO-01: システムは自然言語入力からユーザーの意図を解析しなければならない
- REQ-DO-02: システムは意図を最も適切な GSDT コマンドにマッピングしなければならない
...
- REQ-NEXT-04: プロジェクトが存在しない場合、システムは `/gsdt:new-project` を提案しなければならない
- REQ-NEXT-05: すべてのフェーズが完了している場合、システムは `/gsdt:complete-milestone` を提案しなければならない

**状態検出ロジック:**
| 状態 | アクション |
|------|----------|
| `.claude/.gsdt-planning/` ディレクトリなし | `/gsdt:new-project` を提案 |
| フェーズに CONTEXT.md がない | `/gsdt:discuss-phase` を実行 |
| フェーズに PLAN.md ファイルがない | `/gsdt:plan-phase` を実行 |
| プランはあるが SUMMARY.md がない | `/gsdt:execute-phase` を実行 |
| 実行済みだが VERIFICATION.md がない | `/gsdt:verify-work` を実行 |
| すべてのフェーズが完了 | `/gsdt:complete-milestone` を提案 |
...
- REQ-POSTVER-03: システムは `/gsdt:verify-work` が対処すべき問題をログに記録しなければならない
...
**コマンド:** `/gsdt:health [--repair]`

**目的:** `.claude/.gsdt-planning/` ディレクトリの整合性を検証し、問題を自動修復します。
...
**実行タイミング:** `/gsdt:execute-phase` の検証ステップの前に自動実行されます。
...
**実行タイミング:** `/gsdt:plan-phase` の末尾、プランチェッカーループの後に自動実行されます。
...
- REQ-CTX-06: コンテキストモニターは GSDT アクティブ vs 非 GSDT アクティブプロジェクトを区別しなければならない
...
**コマンド:** `/gsdt:pause-work`、`/gsdt:resume-work`、`/gsdt:progress`
...
**コマンド:** `/gsdt:set-profile <quality|balanced|budget|inherit>`
...
**コマンド:** `/gsdt:map-codebase [area]`

**目的:** 新しいプロジェクトを開始する前に既存のコードベースを分析し、GSDT が既存の構成を理解できるようにします。

**要件:**
- REQ-MAP-01: システムは各分析領域に対して並列マッパーエージェントを起動しなければならない
- REQ-MAP-02: システムは `.claude/.gsdt-planning/codebase/` に構造化されたドキュメントを生成しなければならない
- REQ-MAP-03: システムは技術スタック、アーキテクチャパターン、コーディング規約、懸念事項を検出しなければならない
- REQ-MAP-04: 後続の `/gsdt:new-project` はコードベースマッピングを読み込み、追加する内容に焦点を当てた質問を行わなければならない
...
**コマンド:** `/gsdt:debug [description]`

**目的:** コンテキストリセットを超えて持続する状態を持つ、体系的なデバッグ。

**要件:**
- REQ-DEBUG-01: システムは `.claude/.gsdt-planning/debug/` にデバッグセッションファイルを作成しなければならない
...
- REQ-DEBUG-05: 解決済みセッションは `.claude/.gsdt-planning/debug/knowledge-base.md` に追記されなければならない
...
**コマンド:** `/gsdt:add-todo [desc]`、`/gsdt:check-todos`

**目的:** セッション中にアイデアやタスクをキャプチャし、後で作業できるようにします。

**要件:**
- REQ-TODO-01: システムは現在の会話コンテキストから Todo をキャプチャしなければならない
- REQ-TODO-02: Todo は `.claude/.gsdt-planning/todos/pending/` に保存されなければならない
- REQ-TODO-03: 完了した Todo は `.claude/.gsdt-planning/todos/done/` に移動されなければならない
...
**目的:** GSDT を最新バージョンに更新し、チェンジログのプレビューを表示します。
...
- REQ-UPDATE-04: システムはローカルで変更されたファイルを `gsdt-local-patches/` にバックアップしなければならない
- REQ-UPDATE-05: `/gsdt:reapply-patches` は更新後にローカルの変更を復元しなければならない
...
- REQ-SETTINGS-02: システムは `.claude/.gsdt-planning/config.json` を更新しなければならない
...
| `planning.commit_docs` | boolean | `true` | `.claude/.gsdt-planning/` ファイルを git にコミット |
...
- REQ-GIT-07: システムは `.claude/.gsdt-planning/` ファイルに対して `commit_docs` 設定を尊重しなければならない
- REQ-GIT-08: システムは `.gitignore` の `.claude/.gsdt-planning/` を自動検出し、コミットをスキップしなければならない
...
**目的:** 6つの異なる AI コーディングエージェントランタイムで GSDT を実行します。
...
- REQ-RUNTIME-05: アンインストールは他の設定に影響を与えることなく、すべての GSDT ファイルをクリーンに削除しなければならない
...
**コマンド:** `/gsdt:profile-user [--questionnaire] [--refresh]`
...
- `/gsdt:dev-preferences` コマンド — 任意のセッションで好みを読み込み
...
- `gsdt-user-profiler` エージェント — セッションデータからの行動分析
...
すべての `/gsdt:progress` 呼び出しで、現在のマイルストーンのすべてのフェーズの未解決項目（pending、skipped、blocked、human_needed）をスキャンします。アクション可能なリンク付きのノンブロッキング警告セクションを表示します。
...
**要件:**
- REQ-DEBT-01: システムは `/gsdt:progress` ですべての過去フェーズの未解決 UAT/検証項目を表面化しなければならない
...
- REQ-DEBT-06: `/gsdt:audit-uat` はすべてのフェーズをスキャンし、項目をテスト可能性別にカテゴリ分けし、人的テストプランを生成しなければならない
...
**コマンド:** `/gsdt:fast [task description]`
...
- REQ-FAST-03: システムは状態の一貫性のためにタスクを `.claude/.gsdt-planning/quick/` で追跡しなければならない
- REQ-FAST-04: リサーチ、マルチステッププランニング、または検証が必要なタスクにシステムを使用してはならない

**`/gsdt:quick` との使い分け:**
- `/gsdt:fast` — 2分以内に実行可能な一文のタスク（タイポ修正、設定変更、小規模な追加）
- `/gsdt:quick` — リサーチ、マルチステッププランニング、または検証が必要なもの

---

### 42. クロス AI ピアレビュー

**コマンド:** `/gsdt:review --phase N [--gemini] [--claude] [--codex] [--all]`
...
- REQ-REVIEW-05: レビューは `/gsdt:plan-phase --reviews` で使用可能でなければならない
...
**コマンド:** `/gsdt:add-backlog <description>`、`/gsdt:review-backlog`、`/gsdt:plant-seed <idea>`
...
- REQ-BACKLOG-02: `/gsdt:discuss-phase` と `/gsdt:plan-phase` が動作するよう、フェーズディレクトリは即座に作成されなければならない
- REQ-BACKLOG-03: `/gsdt:review-backlog` は項目ごとにプロモート、維持、削除のアクションをサポートしなければならない
- REQ-BACKLOG-04: プロモートされた項目はアクティブなマイルストーンシーケンスに再番号付けされなければならない
- REQ-SEED-01: シードは完全な WHY と表面化条件の WHEN をキャプチャしなければならない
- REQ-SEED-02: `/gsdt:new-milestone` はシードをスキャンして一致するものを提示しなければならない

**生成物:**
| 成果物 | 説明 |
|--------|------|
| `.claude/.gsdt-planning/phases/999.x-slug/` | バックログ項目ディレクトリ |
| `.claude/.gsdt-planning/seeds/SEED-NNN-slug.md` | トリガー条件付きシード |

---

### 44. 永続コンテキストスレッド

**コマンド:** `/gsdt:thread [name | description]`

**目的:** 複数セッションにまたがるが特定のフェーズには属さない作業のための、軽量なクロスセッションナレッジストア。`/gsdt:pause-work` よりも軽量 — フェーズ状態やプランコンテキストは不要です。

**要件:**
- REQ-THREAD-01: システムは作成、一覧、再開モードをサポートしなければならない
- REQ-THREAD-02: スレッドは `.claude/.gsdt-planning/threads/` にマークダウンファイルとして保存されなければならない
...
**生成物:** `.claude/.gsdt-planning/threads/{slug}.md` — 永続コンテキストスレッド

---

### 45. PR ブランチフィルタリング

**コマンド:** `/gsdt:pr-branch [target branch]`

**目的:** `.claude/.gsdt-planning/` のコミットを除外して、プルリクエストに適したクリーンなブランチを作成します。レビュアーにはコード変更のみが表示され、GSDT プランニング成果物は表示されません。

**要件:**
- REQ-PRBRANCH-01: システムは `.claude/.gsdt-planning/` ファイルのみを変更するコミットを特定しなければならない
...
**目的:** GSDT のプランニング成果物に対する多層防御セキュリティ。GSDT は LLM のシステムプロンプトとなるマークダウンファイルを生成するため、これらのファイルに流入するユーザー制御テキストは間接的なプロンプトインジェクションの潜在的なベクターです。
...
**2. プロンプトインジェクションガードフック**（`gsdt-prompt-guard.js`）
`.claude/.gsdt-planning/` を対象とする Write/Edit 呼び出しをインジェクションパターンでスキャンする PreToolUse フック。アドバイザリーのみ — 正当な操作をブロックせず、検出を認識のためにログ記録します。

**3. ワークフローガードフック**（`gsdt-workflow-guard.js`）
Claude が GSDT ワークフローコンテキスト外でファイル編集を試行した際に検出する PreToolUse フック。直接編集の代わりに `/gsdt:quick` や `/gsdt:fast` の使用をアドバイスします。`hooks.workflow_guard`（デフォルト: false）で設定可能です。
...
**目的:** モノレポおよびマルチリポ構成のための自動検出とプロジェクトルート解決。`.claude/.gsdt-planning/` がリポジトリ境界を超えて解決する必要がある場合のワークスペースをサポートします。
...
**目的:** `/gsdt:discuss-phase` 中に `DISCUSSION-LOG.md` を自動生成し、ディスカッション中の決定事項の完全な監査証跡を残します。
...
**コマンド:** `/gsdt:forensics [description]`

**目的:** 失敗または停滞した GSDT ワークフローのポストモーテム調査。
...
- REQ-FORENSICS-03: システムは `.claude/.gsdt-planning/forensics/` に保存されるマークダウンレポートを生成しなければならない
...
| 成果物 | 説明 |
|--------|------|
| `.claude/.gsdt-planning/forensics/report-{timestamp}.md` | ポストモーテム調査レポート |

**プロセス:**
1. **スキャン** — git 履歴の異常を分析: 停滞ループ、コミット間の長いギャップ、繰り返しの同一コミット
2. **整合性チェック** — 完了したフェーズに期待される成果物ファイルがあるか検証
3. **レポート** — 調査結果を含むマークダウンレポートを生成し、`.claude/.gsdt-planning/forensics/` に保存
...
**コマンド:** `/gsdt:milestone-summary [version]`
...
**コマンド:** `/gsdt:workstreams`

**目的:** 異なるマイルストーン領域での同時作業のための並列ワークストリーム。

**要件:**
- REQ-WS-01: システムはワークストリーム状態を個別の `.claude/.gsdt-planning/workstreams/{name}/` ディレクトリに分離しなければならない
...
| 成果物 | 説明 |
|--------|------|
| `.claude/.gsdt-planning/workstreams/{name}/` | 分離されたワークストリームディレクトリ構造 |

**プロセス:**
1. **作成** — 分離された `.claude/.gsdt-planning/workstreams/{name}/` ディレクトリで名前付きワークストリームを初期化
2. **切り替え** — 後続の GSDT コマンドのためにアクティブなワークストリームコンテキストを変更
...
**コマンド:** `/gsdt:discuss-phase`（`workflow.discuss_mode: 'assumptions'` 設定時）
...
**対象:** `/gsdt:new-project` および `/gsdt:progress`

**目的:** UI 重視のプロジェクトを自動検出し、`/gsdt:ui-phase` の推奨を表面化します。
...
- REQ-UI-DETECT-03: システムは UI 重視フェーズのネクストステップに `/gsdt:ui-phase` を提案しなければならない
- REQ-UI-DETECT-04: システムは `/gsdt:ui-phase` を必須にしてはならない

**プロセス:**
1. **検出** — プロジェクト説明と技術スタックの UI シグナル（キーワード、フレームワーク参照）をスキャン
2. **アノテーション** — ROADMAP.md の該当フェーズに `ui_hint` マーカーを追加
3. **表面化** — UI 重視フェーズのネクストステップに `/gsdt:ui-phase` の推奨を含める
...
3. **インストール** — 1回のセッションで選択されたすべてのランタイムに対して GSDT を設定
