...
- REQ-INIT-06: `.gsdt-planning/PROJECT.md` が既に存在する場合、システムは再初期化を防止しなければならない
...
- REQ-UIREVIEW-02: システムは Playwright CLI を使用して `.gsdt-planning/ui-reviews/` にスクリーンショットをキャプチャしなければならない
...
- REQ-QUICK-08: システムは Quick タスクを `.gsdt-planning/quick/YYMMDD-xxx-slug/` で追跡しなければならない
...
| 状態 | アクション |
|------|----------|
| `.gsdt-planning/` ディレクトリなし | `/gsdt:new-project` を提案 |
...
**目的:** `.gsdt-planning/` ディレクトリの整合性を検証し、問題を自動修復します。
...
- REQ-MAP-02: システムは `.gsdt-planning/codebase/` に構造化されたドキュメントを生成しなければならない
...
**要件:**
- REQ-DEBUG-01: システムは `.gsdt-planning/debug/` にデバッグセッションファイルを作成しなければならない
...
- REQ-DEBUG-05: 解決済みセッションは `.gsdt-planning/debug/knowledge-base.md` に追記されなければならない
...
- REQ-TODO-02: Todo は `.gsdt-planning/todos/pending/` に保存されなければならない
- REQ-TODO-03: 完了した Todo は `.gsdt-planning/todos/done/` に移動されなければならない
...
- REQ-GIT-07: システムは `.gsdt-planning/` ファイルに対して `commit_docs` 設定を尊重しなければならない
- REQ-GIT-08: システムは `.gitignore` の `.gsdt-planning/` を自動検出し、コミットをスキップしなければならない
...
- REQ-FAST-03: システムは状態の一貫性のためにタスクを `.gsdt-planning/quick/` で追跡しなければならない
...
| 成果物 | 説明 |
|--------|------|
| `.gsdt-planning/phases/999.x-slug/` | バックログ項目ディレクトリ |
| `.gsdt-planning/seeds/SEED-NNN-slug.md` | トリガー条件付きシード |
...
- REQ-THREAD-02: スレッドは `.gsdt-planning/threads/` にマークダウンファイルとして保存されなければならない
...
**生成物:** `.gsdt-planning/threads/{slug}.md` — 永続コンテキストスレッド
...
**目的:** `.gsdt-planning/` のコミットを除外して、プルリクエストに適したクリーンなブランチを作成します。レビュアーにはコード変更のみが表示され、GSDT プランニング成果物は表示されません。

**要件:**
- REQ-PRBRANCH-01: システムは `.gsdt-planning/` ファイルのみを変更するコミットを特定しなければならない
...
`.gsdt-planning/` を対象とする Write/Edit 呼び出しをインジェクションパターンでスキャンする PreToolUse フック。アドバイザリーのみ — 正当な操作をブロックせず、検出を認識のためにログ記録します。
...
**目的:** モノレポおよびマルチリポ構成のための自動検出とプロジェクトルート解決。`.gsdt-planning/` がリポジトリ境界を超えて解決する必要がある場合のワークスペースをサポートします。
...
- REQ-FORENSICS-03: システムは `.gsdt-planning/forensics/` に保存されるマークダウンレポートを生成しなければならない
...
| 成果物 | 説明 |
|--------|------|
| `.gsdt-planning/forensics/report-{timestamp}.md` | ポストモーテム調査レポート |

**プロセス:**
1. **スキャン** — git 履歴の異常を分析: 停滞ループ、コミット間の長いギャップ、繰り返しの同一コミット
2. **整合性チェック** — 完了したフェーズに期待される成果物ファイルがあるか検証
3. **レポート** — 調査結果を含むマークダウンレポートを生成し、`.gsdt-planning/forensics/` に保存
...
**要件:**
- REQ-WS-01: システムはワークストリーム状態を個別の `.gsdt-planning/workstreams/{name}/` ディレクトリに分離しなければならない
...
| 成果物 | 説明 |
|--------|------|
| `.gsdt-planning/workstreams/{name}/` | 分離されたワークストリームディレクトリ構造 |

**プロセス:**
1. **作成** — 分離された `.gsdt-planning/workstreams/{name}/` ディレクトリで名前付きワークストリームを初期化
