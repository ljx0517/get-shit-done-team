...
| CRITICAL | <= 25% | 即座に停止し、状態を保存する（`/gsdt:pause-work`） |
...
```
ステータスラインフック (gsdt-statusline.js)
    | 書き込み
    v
/tmp/claude-ctx-{session_id}.json
    ^ 読み取り
    |
コンテキストモニター (gsdt-context-monitor.js, PostToolUse/AfterTool)
    | 注入
    v
...
両フックは `npx gsdt` のインストール時に自動的に登録されます:
