# Referência de Recursos do GSDT

Visão em Português dos recursos centrais do GSDT.  
...
- **UAT guiado** via `/gsdt:verify-work`
- **Suporte brownfield** com `/gsdt:map-codebase`
...
- **`/gsdt:ui-phase`**: contrato visual antes da execução
- **`/gsdt:ui-review`**: auditoria visual em 6 pilares
...
- **Diagnóstico forense** com `/gsdt:forensics`
- **Relatório de sessão** com `/gsdt:session-report`
...
| Cenário | Comandos |
|--------|----------|
| Projeto novo | `/gsdt:new-project` -> `/gsdt:discuss-phase` -> `/gsdt:plan-phase` -> `/gsdt:execute-phase` |
| Correção rápida | `/gsdt:quick` |
| Código existente | `/gsdt:map-codebase` -> `/gsdt:new-project` |
| Fechamento de release | `/gsdt:audit-milestone` -> `/gsdt:complete-milestone` |
