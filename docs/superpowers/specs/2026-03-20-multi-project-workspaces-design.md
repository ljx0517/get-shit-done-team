...
GSD is tied to one `.gsdt-planning/` directory per working directory. Users with multiple independent projects (monorepo-style setups with 20+ child repos) or users needing feature branch isolation in the same repo cannot run parallel GSD sessions without manual cloning and state management.
...
Scans `~/gsdt-workspaces/*/WORKSPACE.md` for workspace manifests. Displays table with name, path, repo count, GSD status (has PROJECT.md, current phase).
...
- `WORKSPACE.md` is the only GSD-specific file at the root (besides `.gsdt-planning/`)
...
| `.gsdt-planning/` at workspace root | Gives full isolation from individual repo planning. Each workspace is an independent GSD project |
