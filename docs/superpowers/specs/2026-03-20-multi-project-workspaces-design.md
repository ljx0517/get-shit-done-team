...
GSDT is tied to one `.claude/.gsdt-planning/` directory per working directory. Users with multiple independent projects (monorepo-style setups with 20+ child repos) or users needing feature branch isolation in the same repo cannot run parallel GSDT sessions without manual cloning and state management.
...
- `WORKSPACE.md` is the only GSDT-specific file at the root (besides `.claude/.gsdt-planning/`)
...
| `.claude/.gsdt-planning/` at workspace root | Gives full isolation from individual repo planning. Each workspace is an independent GSDT project |
