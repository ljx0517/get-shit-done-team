...
```
  ...
  └─────────────────────────┬────────────────────────┘
             ...
             │                            │
             │  ┌────────────────────┐    │
             │  │ /gsdt:discuss-phase │    │  <- 锁定偏好
             │  └──────────┬─────────┘    │
             │             │              │
             │  ┌──────────▼─────────┐    │
             │  │ /gsdt:plan-phase    │    │  <- 研究 + 规划 + 验证
             │  └──────────┬─────────┘    │
             │             │              │
             │  ┌──────────▼─────────┐    │
             │  │ /gsdt:execute-phase │    │  <- 并行执行
             │  └──────────┬─────────┘    │
             │             │              │
             │  ┌──────────▼─────────┐    │
             │  │ /gsdt:verify-work   │    │  <- 手动 UAT
             │  └──────────┬─────────┘    │
             │             │              │
             │     下一阶段?────────────┘
             │             │ 否
...
**禁用：** 在 `/gsdt:settings` 中设置 `workflow.nyquist_validation: false`，用于测试基础设施不是重点的快速原型阶段。

### 追溯验证 (`/gsdt:validate-phase`)
...
**何时使用：** 在启用了 Nyquist 之前规划的阶段执行后，或在 `/gsdt:audit-milestone` 发现 Nyquist 合规缺口后。
...
| 命令 | 用途 | 何时使用 |
|---------|---------|-------------|
| `/gsdt:new-project` | 完整项目初始化：提问、研究、需求、路线图 | 新项目开始时 |
| `/gsdt:new-project --auto @idea.md` | 从文档自动初始化 | 有现成的 PRD 或想法文档 |
| `/gsdt:discuss-phase [N]` | 捕获实现决策 | 规划前，塑造构建方式 |
| `/gsdt:plan-phase [N]` | 研究 + 规划 + 验证 | 执行阶段前 |
| `/gsdt:execute-phase <N>` | 在并行波次中执行所有计划 | 规划完成后 |
| `/gsdt:verify-work [N]` | 带自动诊断的手动 UAT | 执行完成后 |
| `/gsdt:audit-milestone` | 验证里程碑达到其完成定义 | 完成里程碑前 |
| `/gsdt:complete-milestone` | 归档里程碑，标记发布 | 所有阶段已验证 |
| `/gsdt:new-milestone [name]` | 开始下一个版本周期 | 完成里程碑后 |

### 导航

| 命令 | 用途 | 何时使用 |
|---------|---------|-------------|
| `/gsdt:progress` | 显示状态和下一步 | 任何时候 -- "我在哪?" |
...
| 命令 | 用途 | 何时使用 |
|---------|---------|-------------|
| `/gsdt:add-phase` | 向路线图追加新阶段 | 初始规划后范围增长 |
| `/gsdt:insert-phase [N]` | 插入紧急工作（小数编号） | 里程碑中途紧急修复 |
| `/gsdt:remove-phase [N]` | 删除未来阶段并重新编号 | 移除某个功能 |
| `/gsdt:list-phase-assumptions [N]` | 预览 Claude 的预期方法 | 规划前，验证方向 |
| `/gsdt:plan-milestone-gaps` | 为审计缺口创建阶段 | 审计发现缺失项后 |
| `/gsdt:research-phase [N]` | 仅深度生态研究 | 复杂或不熟悉的领域 |
...
GSD 在 `.gsdt-planning/config.json` 中存储项目设置。在 `/gsdt:new-project` 期间配置或稍后用 `/gsdt:settings` 更新。
...
/gsdt:execute-phase 1        # 并行执行
/gsdt:verify-work 1          # 手动 UAT
/clear
/gsdt:discuss-phase 2        # 对每个阶段重复
...
/gsdt:audit-milestone        # 检查所有内容已发布
/gsdt:complete-milestone     # 归档，标记，完成
```

### 从现有文档创建新项目

```bash
/gsdt:new-project --auto @prd.md   # 从你的文档自动运行研究/需求/路线图
/clear
/gsdt:discuss-phase 1               # 从这里开始正常流程
```
...
```bash
/gsdt:progress               # 查看你停在哪和接下来做什么
# 或
/gsdt:resume-work            # 从上次会话完整恢复上下文
```

### 准备发布

```bash
/gsdt:audit-milestone        # 检查需求覆盖率，检测存根
/gsdt:plan-milestone-gaps    # 如果审计发现缺口，创建阶段来填补
/gsdt:complete-milestone     # 归档，标记，完成
```
...
```bash
/gsdt:add-phase              # 向路线图追加新阶段
# 或
/gsdt:insert-phase 3         # 在阶段 3 和 4 之间插入紧急工作
# 或
/gsdt:remove-phase 7         # 移除阶段 7 并重新编号
```
...
在规划前运行 `/gsdt:discuss-phase [N]`。大多数计划质量问题来自 Claude 做出了 `CONTEXT.md` 本可以防止的假设。你也可以运行 `/gsdt:list-phase-assumptions [N]` 在提交计划前查看 Claude 打算做什么。
...
运行 `/gsdt:progress`。它读取所有状态文件，准确告诉你位置和下一步。

### 执行后需要更改某些内容

不要重新运行 `/gsdt:execute-phase`。使用 `/gsdt:quick` 进行针对性修复，或用 `/gsdt:verify-work` 通过 UAT 系统识别和修复问题。

### 模型成本太高

切换到 budget 配置：`/gsdt:set-profile budget`。如果领域对你（或 Claude）熟悉，通过 `/gsdt:settings` 禁用研究和计划检查代理。
...
| 问题 | 解决方案 |
|---------|----------|
| 丢失上下文 / 新会话 | `/gsdt:resume-work` 或 `/gsdt:progress` |
| 阶段出错 | `git revert` 阶段提交，然后重新规划 |
| 需要更改范围 | `/gsdt:add-phase`、`/gsdt:insert-phase` 或 `/gsdt:remove-phase` |
| 里程碑审计发现缺口 | `/gsdt:plan-milestone-gaps` |
| 出问题了 | `/gsdt:debug "描述"` |
| 快速针对性修复 | `/gsdt:quick` |
| 计划与你的愿景不符 | `/gsdt:discuss-phase [N]` 然后重新规划 |
| 成本过高 | `/gsdt:set-profile budget` 和 `/gsdt:settings` 关闭代理 |
| 更新破坏了本地更改 | `/gsdt:reapply-patches` |
...
.gsdt-planning/
  ...
  research/               # 来自 /gsdt:new-project 的领域研究
  todos/
    pending/              # 等待处理的捕获想法
    done/                 # 已完成的待办事项
  debug/                  # 活跃调试会话
    resolved/             # 已归档的调试会话
  codebase/               # 现有代码库映射（来自 /gsdt:map-codebase）
  phases/
    XX-phase-name/
