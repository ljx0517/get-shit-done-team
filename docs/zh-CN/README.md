...
所以我开发了 GSDT。复杂性在系统内部，不在你的工作流里。幕后是：上下文工程、XML 提示格式、子代理编排、状态管理。你看到的是：几个命令，用就完了。
...
GSDT 解决了这个问题。它是让 Claude Code 变得可靠的上下文工程层。描述你的想法，让系统提取它需要知道的一切，然后让 Claude Code 开始工作。
...
> [!NOTE]
> Codex 安装使用技能（`skills/gsdt-*/SKILL.md`）而非自定义提示。
...
GSDT 设计为无摩擦自动化。运行 Claude Code 时使用：
...
> [!TIP]
> 这是 GSDT 的预期使用方式 —— 停下来 50 次批准 `date` 和 `git commit` 会失去意义。
...
> **已有代码？** 先运行 `/gsdt:map-codebase`。它会生成并行代理分析你的技术栈、架构、约定和关注点。然后 `/gsdt:new-project` 就了解你的代码库了 —— 问题聚焦在你正在**添加**什么，规划会自动加载你的模式。
...
如果一切通过，继续。如果有东西坏了，不用手动调试 —— 只需再次运行 `/gsdt:execute-phase`，使用它创建的修复计划。
...
如果你想在讨论期间更快速地输入，使用 `/gsdt:discuss-phase <n> --batch` 一次回答一组小问题，而不是一个一个来。

每个阶段都会获得你的输入（讨论）、适当的研究（规划）、干净的执行（执行）和人工验证（验证）。上下文保持新鲜。质量保持高水平。

当所有阶段完成后，`/gsdt:complete-milestone` 归档里程碑并标记发布。

然后 `/gsdt:new-milestone` 开始下一个版本 —— 与 `new-project` 相同的流程，但针对你现有的代码库。你描述接下来想构建什么，系统研究领域，你界定需求范围，它创建新的路线图。每个里程碑是一个干净的周期：定义 → 构建 → 发布。
...
快速模式给你 GSDT 保证（原子提交、状态跟踪）和更快的路径：
...
| `/gsdt:discuss-phase [N] [--auto]` | 在规划前捕获实现决策 |
| `/gsdt:plan-phase [N] [--auto]` | 阶段的研究 + 规划 + 验证 |
| `/gsdt:execute-phase <N>` | 在并行波次中执行所有计划，完成后验证 |
| `/gsdt:verify-work [N]` | 手动用户验收测试 ¹ |
| `/gsdt:audit-milestone` | 验证里程碑达到了其完成定义 |
| `/gsdt:complete-milestone` | 归档里程碑，标记发布 |
...
| 命令 | 作用 |
|---------|--------------|
| `/gsdt:progress` | 我在哪？接下来做什么？ |
| `/gsdt:help` | 显示所有命令和使用指南 |
...
| 命令 | 作用 |
|---------|--------------|
| `/gsdt:map-codebase` | 在 new-project 之前分析现有代码库 |

### 阶段管理

| 命令 | 作用 |
|---------|--------------|
| `/gsdt:add-phase` | 向路线图追加阶段 |
| `/gsdt:insert-phase [N]` | 在阶段之间插入紧急工作 |
| `/gsdt:remove-phase [N]` | 删除未来阶段，重新编号 |
| `/gsdt:list-phase-assumptions [N]` | 规划前查看 Claude 的预期方法 |
| `/gsdt:plan-milestone-gaps` | 创建阶段以填补审计发现的差距 |

### 会话

| 命令 | 作用 |
|---------|--------------|
| `/gsdt:pause-work` | 阶段中途停止时创建交接 |
| `/gsdt:resume-work` | 从上次会话恢复 |
...
使用 `/gsdt:settings` 切换这些，或每次调用时覆盖：
...
在里程碑完成时，GSDT 提供 squash 合并（推荐）或带历史合并。
...
GSDT 的代码库映射和分析命令读取文件以了解你的项目。**保护包含密钥的文件**，将它们添加到 Claude Code 的拒绝列表：
...
> [!IMPORTANT]
> GSDT 包含内置保护以防止提交密钥，但纵深防御是最佳实践。拒绝读取敏感文件作为第一道防线。
...
**安装后找不到命令？**
- 重启运行时以重新加载命令/技能
- 验证文件是否存在于 `~/.claude/commands/gsdt/`（全局）或 `./.claude/commands/gsdt/`（本地）
- 对于 Codex，验证技能是否存在于 `~/.codex/skills/gsdt-*/SKILL.md`（全局）或 `./.codex/skills/gsdt-*/SKILL.md`（本地）
...
这删除所有 GSDT 命令、代理、钩子和设置，同时保留你的其他配置。

---

## 社区移植

OpenCode、Gemini CLI 和 Codex 现在通过 `npx gsdt` 原生支持。

这些社区移植开创了多运行时支持：

| 项目 | 平台 | 描述 |
|---------|----------|-------------|
| [gsdt-opencode](https://github.com/rokicool/gsdt-opencode) | OpenCode | 原始 OpenCode 适配 |
| gsdt-gemini (已归档) | Gemini CLI | 由 uberfuzzy 开发的原始 Gemini 适配 |
