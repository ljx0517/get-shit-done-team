...
1. Does the project already exist? (has `.gsdt-planning/PROJECT.md`)
2. Does it have a roadmap? (has `.gsdt-planning/ROADMAP.md`)
3. Does it have recent state? (has `.gsdt-planning/STATE.md`)
...
function determineClassifyContext(projectDir: string): ClassifyContext {
  const hasProject = exists(join(projectDir, '.gsdt-planning', 'PROJECT.md'));
  const hasRoadmap = exists(join(projectDir, '.gsdt-planning', 'ROADMAP.md'));
  const hasState = exists(join(projectDir, '.gsdt-planning', 'STATE.md'));
