...
> [!NOTE]
> Codex 설치는 커스텀 프롬프트 대신 스킬(`skills/gsdt-*/SKILL.md`)을 사용합니다.
...
# Codex (스킬 우선)
npx gsdt --codex --global    # ~/.codex/에 설치
npx gsdt --codex --local     # ./.codex/에 설치

# Copilot (GitHub Copilot CLI)
npx gsdt --copilot --global  # ~/.github/에 설치
npx gsdt --copilot --local   # ./.github/에 설치

# Cursor CLI
npx gsdt --cursor --global      # ~/.cursor/에 설치
npx gsdt --cursor --local       # ./.cursor/에 설치

# Antigravity (Google, 스킬 우선, Gemini 기반)
npx gsdt --antigravity --global # ~/.gemini/antigravity/에 설치
npx gsdt --antigravity --local  # ./.agent/에 설치

# 전체 런타임
npx gsdt --all --global      # 모든 디렉터리에 설치
```
...
> **이미 코드가 있나요?** 먼저 `/gsdt:map-codebase`를 실행하세요. 병렬 에이전트를 생성해 스택, 아키텍처, 컨벤션, 고려사항을 분석합니다. 그러면 `/gsdt:new-project`가 코드베이스를 파악한 상태에서 시작되고 — 질문은 추가하는 것에 집중되고, 기획 시 자동으로 기존 패턴을 불러옵니다.
...
> **가정 모드:** 질문보다 코드베이스 분석을 선호하나요? `/gsdt:settings`에서 `workflow.discuss_mode`를 `assumptions`로 설정하세요. 시스템이 코드를 읽고 하려는 것과 이유를 제시한 다음 틀린 부분만 수정을 요청합니다. [논의 모드](docs/ko-KR/workflow-discuss-mode.md) 참조.
...
모든 게 통과하면 다음으로 넘어갑니다. 뭔가 깨졌으면 직접 디버그하지 않아도 됩니다 — 생성된 수정 계획으로 `/gsdt:execute-phase`만 다시 실행하면 됩니다.
...
논의 중에 더 빠르게 진행하고 싶다면 `/gsdt:discuss-phase <n> --batch`를 사용해 하나씩이 아닌 소그룹으로 한 번에 답할 수 있습니다.

각 단계는 사용자 입력(논의), 적절한 리서치(기획), 깔끔한 실행(실행), 사람의 검증(검증)을 거칩니다. 컨텍스트는 새롭게 유지됩니다. 품질도 높게 유지됩니다.

모든 단계가 끝나면 `/gsdt:complete-milestone`이 마일스톤을 아카이브하고 릴리스에 태그를 답니다.

그다음 `/gsdt:new-milestone`으로 다음 버전을 시작합니다 — `new-project`와 같은 흐름이지만 기존 코드베이스를 위한 것입니다. 다음에 만들 것을 설명하면 시스템이 도메인을 리서치하고, 요구사항을 스코핑하고, 새 로드맵을 만듭니다. 각 마일스톤은 깔끔한 사이클입니다: 정의 → 구축 → 출시.
...
| 명령어 | 역할 |
|---------|------------|
| `/gsdt:new-project [--auto]` | 전체 초기화: 질문 → 리서치 → 요구사항 → 로드맵 |
| `/gsdt:discuss-phase [N] [--auto] [--analyze]` | 기획 전 구현 결정 캡처 (`--analyze`는 트레이드오프 분석 추가) |
| `/gsdt:plan-phase [N] [--auto] [--reviews]` | 단계에 대한 리서치 + 기획 + 검증 (`--reviews`는 코드베이스 리뷰 결과 로드) |
| `/gsdt:execute-phase <N>` | 병렬 웨이브로 모든 계획 실행, 완료 시 검증 |
| `/gsdt:verify-work [N]` | 수동 사용자 인수 테스트 ¹ |
| `/gsdt:ship [N] [--draft]` | 자동 생성된 본문으로 검증된 단계 작업에서 PR 생성 |
| `/gsdt:next` | 다음 논리적 워크플로우 단계로 자동 진행 |
| `/gsdt:fast <text>` | 인라인 사소한 작업 — 기획 완전 건너뛰고 즉시 실행 |
| `/gsdt:audit-milestone` | 마일스톤이 완료 정의를 달성했는지 검증 |
| `/gsdt:complete-milestone` | 마일스톤 아카이브, 릴리스 태그 |
| `/gsdt:new-milestone [name]` | 다음 버전 시작: 질문 → 리서치 → 요구사항 → 로드맵 |
| `/gsdt:forensics [desc]` | 실패한 워크플로우 실행의 사후 조사 (막힌 루프, 누락된 아티팩트, git 이상 진단) |
| `/gsdt:milestone-summary [version]` | 팀 온보딩 및 리뷰를 위한 종합 프로젝트 요약 생성 |

### 워크스트림

| 명령어 | 역할 |
|---------|------------|
| `/gsdt:workstreams list` | 모든 워크스트림과 상태 표시 |
| `/gsdt:workstreams create <name>` | 병렬 마일스톤 작업을 위한 네임스페이스 워크스트림 생성 |
| `/gsdt:workstreams switch <name>` | 활성 워크스트림 전환 |
| `/gsdt:workstreams complete <name>` | 워크스트림 완료 및 병합 |
...
| 명령어 | 역할 |
|---------|------------|
| `/gsdt:ui-phase [N]` | 프론트엔드 단계를 위한 UI 디자인 계약 (UI-SPEC.md) 생성 |
| `/gsdt:ui-review [N]` | 구현된 프론트엔드 코드의 소급적 6가지 기준 시각 감사 |
...
| 명령어 | 역할 |
|---------|------------|
| `/gsdt:map-codebase [area]` | new-project 전 기존 코드베이스 분석 |

### 단계 관리

| 명령어 | 역할 |
|---------|------------|
| `/gsdt:add-phase` | 로드맵에 단계 추가 |
| `/gsdt:insert-phase [N]` | 단계 사이에 긴급 작업 삽입 |
| `/gsdt:remove-phase [N]` | 미래 단계 제거, 번호 재정렬 |
| `/gsdt:list-phase-assumptions [N]` | 기획 전 Claude의 의도된 접근 방식 확인 |
| `/gsdt:plan-milestone-gaps` | 감사에서 발견된 갭을 해소하기 위한 단계 생성 |

### 세션

| 명령어 | 역할 |
|---------|------------|
| `/gsdt:pause-work` | 단계 중간에 멈출 때 핸드오프 생성 (HANDOFF.json 작성) |
| `/gsdt:resume-work` | 마지막 세션에서 복원 |
| `/gsdt:session-report` | 수행한 작업과 결과가 담긴 세션 요약 생성 |
...
| 명령어 | 역할 |
|---------|------------|
| `/gsdt:plant-seed <idea>` | 트리거 조건이 있는 아이디어 저장 — 때가 되면 알아서 올라옴 |
| `/gsdt:add-backlog <desc>` | 백로그 파킹 롯에 아이디어 추가 (999.x 번호 지정, 활성 시퀀스 외부) |
| `/gsdt:review-backlog` | 백로그 항목 리뷰 및 활성 마일스톤으로 승격하거나 오래된 항목 제거 |
| `/gsdt:thread [name]` | 지속적 컨텍스트 스레드 — 여러 세션에 걸친 작업을 위한 가벼운 크로스 세션 지식 |

### 유틸리티

| 명령어 | 역할 |
|---------|------------|
| `/gsdt:settings` | 모델 프로필 및 워크플로우 에이전트 설정 |
| `/gsdt:set-profile <profile>` | 모델 프로필 전환 (quality/balanced/budget/inherit) |
...
`/gsdt:settings`로 토글하거나 호출별로 재정의하세요:
...
- `~/.claude/commands/gsdt/` (전역) 또는 `./.claude/commands/gsdt/` (로컬)에 파일이 있는지 확인하세요
- Codex의 경우 `~/.codex/skills/gsdt-*/SKILL.md` (전역) 또는 `./.codex/skills/gsdt-*/SKILL.md` (로컬)에 스킬이 있는지 확인하세요

**명령어가 예상대로 작동하지 않나요?**
- `/gsdt:help`를 실행해 설치 확인
- `npx gsdt`를 다시 실행해 재설치
